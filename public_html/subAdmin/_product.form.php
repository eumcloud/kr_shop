...
<form name="frm" method="post" ENCTYPE="multipart/form-data" action="_product.pro.php">
	...
	<input type=hidden name="pro_idx" value="<?=($row[pro_idx]?$row[pro_idx]: 999)?>">
		...
		<table class="form_TB" summary="검색항목">
			...
			<tbody>
				<tr>
					...
					<td class="conts">
						<?=($_mode == "modify" ? ($row[p_view] == "Y" ? "노출상태" : _DescStr("등록된 상품은 숨김처리되며, 관리자 검수 후 노출처리됩니다.")) : _DescStr("등록된 상품은 숨김처리되며, 관리자 검수 후 노출처리됩니다."))?>
					</td>
				</tr>
					...
					<td class="conts">
						...
						<div ID="setup_delivery_apply" style='display:<?=($row['setup_delivery']=="Y" ?"":"none")?>'>
							...
							<div ID="del_type_normal" style='display:<?=($row['del_type']=="normal" ?"":"none")?>'>
								<?if(sizeof($com) > 0 ):?>
								<br>&nbsp;업체 기본배송비 : <U><?=($com['com_delprice'] ? number_format($com['com_delprice']) . "원" : "0원")?></U>
								<br>&nbsp;업체 무료배송비 : <U><?=($com['com_delprice_free'] ? number_format($com['com_delprice_free']) . "원" : "무조건 배송비 부과")?></U>
								<br>&nbsp;업체 지정택배사 : <U><?=($com['com_del_company'] ? $com['com_del_company'] : "-미정-")?></U>
								<?endif;?>
							</div>
               ...
               <tr><!-- LMH003 -->
               	...
               	<? } ?>
               		주소 : <input type="text" name="com_juso" id="com_juso" value="<?=($row[com_juso] ? $row[com_juso] : htmlspecialchars(trim($com[address] ." ". $com[address1])))?>" size="85" class="input_text"/>
