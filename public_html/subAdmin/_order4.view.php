<?PHP
# LDD007
$app_current_link = "/subAdmin/_order4.list.php";
	...
	<table class="form_TB" summary="검색항목">
		...
		<tbody>
			<tr>
				...
				<td class="conts"><?php echo date('Y-m-d', strtotime($r['s_date'])); ?></td>
			</tr>
			<tr>
				<td class="article">총 금액</td>
				<td class="conts"><?php echo number_format($r['s_price']); ?>원</td>
			</tr>
			<tr>
				<td class="article">정산수량</td>
				<td class="conts"><?php echo number_format($r['s_count']); ?></td>
			</tr>
			<tr>
			...
			foreach($pr as $sk=>$sv) {
				...
				if($sv[op_delivstatus] == "N") { 
					...
					if($sv[op_orderproduct_type] == "coupon") {
						...
						$status_print .= "<br><B><a href='".$arr_delivery_company[$sv[op_expressname]].$sv[op_expressnum]."' target='_blank' title='' >[배송조회]</a></B>";
					}
				}
			...
			<tr>
				...
				<td>
					<div class="btn_line_up_center view_bt">
						<span class="shop_btn_pack"><input type="button" onclick="location.href='_order.view.php?_mode=modify&ordernum=<?php echo $sv[op_oordernum]; ?>';" value="상세보기" class="input_small blue"></span>
					</div>
				</td>
			</tr>
			<?php } ?>
