<?PHP
	...
	$app_current_link = "/subAdmin/_return.list.php";
    ...
    $r = _MQ("
    	...
    	WHERE rr.rr_uid = '".$uid."' and op.op_partnerCode = '".$com[id]."'
    ");
...
<?PHP
	...
	echo "
			...
			"._DescStr("교환/반품 처리는 주문과 연동되지 않습니다.")."
		</div>
	";
?>
				...
				<table class="form_TB" summary="검색항목">
					...
					<tbody>
						...
						<tr>
							...
							<td class="conts">
								<textarea name="_content" class="input_text" readonly style="width:100%;height:100px;" ><?=stripslashes($r[rr_content])?></textarea>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<table class="form_TB" summary="검색항목">
					...
					<tbody>
						<tr>
							...
							<td class="conts">
								<label><input type="radio" name="_status" value="P" <?="P"==$r[rr_status]?'checked':''?>/> 처리중</label>&nbsp;&nbsp;&nbsp;
								<label><input type="radio" name="_status" value="Y" <?="Y"==$r[rr_status]?'checked':''?>/> 완료</label>
