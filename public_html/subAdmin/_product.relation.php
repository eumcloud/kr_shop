<?PHP
	...
	// 넘겨저온 변수
	//		code (필수)
	//		relation_procode (선택) - 입력예: 상품코드1/상품코드2/상품코드3 (상품코드의 구분을 / 로 해주시기 바랍니다.)
					...
					<table class="list_TB" summary="리스트기본">
						<thead>
							<tr>
								...
								<th scope="col" class="colorset"><input type=checkbox name='allcheck' value='Y' onclick="selectAll();" <?echo (sizeof($relation_procode_Division) > 0) ? "checked" : ""; ?>></th>
...
<?PHP
	...
	if( !$cateCode && !$pass_parent02 && !$pass_parent01 ) {
		echo "<tr><td colspan=6 height='80'>해당분류를 선택해 주시기 바랍니다.</td></tr>";
	}
	else {

		$s_query = " where p.customerCode='". $com[id] ."' ";
		if($code) { $s_query .= " and p.code != '" . $code ."' "; }
		$relation_procode_Division = array_filter(explode("/",$relation_procode));
		if($relation_procode) { $s_query .= " and p.code in ('".implode("','",$relation_procode_Division)."') "; }
