<?PHP
	...
	if( $pass_rechtel ) { $s_query .= " AND ( IF(op_orderproduct_type = 'coupon' , concat(ordertel1 ,'-',ordertel2 ,'-',ordertel3) , concat(rectel1 ,'-',rectel2 ,'-',rectel3)) like '%". $pass_rechtel ."%' or IF(op_orderproduct_type = 'coupon' , concat(userhtel1 ,'-',userhtel2 ,'-',userhtel3) , concat(rechtel1 ,'-',rechtel2 ,'-',rechtel3)) like '%". $pass_rechtel ."%'  ) "; }//수령자연락처
						...
						<tbody>
							...
							<tr>
								...
								<td class="conts" colspan="3">
									<a href="_order2.reserve_list.php"><?php echo number_format(reserve_order('', $com['id'])); ?> 건</a>
...
<?PHP
    ...
    $que = "
        SELECT 
			...
			concat(userhtel1 ,'-',userhtel2 ,'-',userhtel3) as userhtel ,
			concat(rectel1 ,'-',rectel2 ,'-',rectel3) as rectel,
			concat(rechtel1 ,'-',rechtel2 ,'-',rechtel3) as rechtel
		FROM odtOrderProduct as op 
        left join odtOrder as o on ( o.ordernum=op.op_oordernum )
...
<input type='hidden' name='opc_uid[]' value='". $i ."'>
			...
			// 모바일 아이콘 LDD002
			$device_icon = '<span class="shop_state_pack" style="display:block"><span class="blue">PC주문</span></span>';
			if($sv['mobile'] == 'Y') $device_icon = '<span class="shop_state_pack" style="display:block"><span class="orange">MOBLIE주문</span></span>';
		...
		$sub_name = ( $sv[op_orderproduct_type]=="coupon" ? $sv[username] : $sv[recname] );
		$sub_name = $sub_name == $sv[ordername] ? "" : "<br>".$sub_name ;

		$sub_tel = ( $sv[op_orderproduct_type]=="coupon" ? $sv[ordertel] : $sv[rectel] );
		$sub_htel = ( $sv[op_orderproduct_type]=="coupon" ? $sv[userhtel] : $sv[rechtel] );
