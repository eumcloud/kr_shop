...
function CurrentItem($cateCode,$slntype,$datepatch,$changeTime) {
    ...
    $Query = "select code from ".$slntype."Product where code = parent_code and cateCode = '".$cateCode."' $add_qry order by sale_date desc limit 1";
...
##반복구문생성
function RunLoop($content,$hourtype="00",$viewcount="99") {
...
//echo $CQuery;
    ...
    while ($CRecord = mysql_fetch_array($CResult))
    {
        ...
        $PID = CurrentItem($AID,$slntype,$datepatch,$basic[changeTime]);
        if(!$PID) continue;
        $PIDN = ereg_replace("[^0-9]", "", $PID);   //PID중 숫자만취함
        ...
        if ( !strpos($PIMG,"://") && $PIMG ) $PIMG ="http://$_SERVER[HTTP_HOST]".$PIMG; //주소값붙인다.
		...
		//모바일용 LINK값
		$MLINK = "http://".$_SERVER[HTTP_HOST]."/m/?pid=$PID";
...
//        echo"<br>BeCnt==>".$CNTSALE;
        $cntQue = "SELECT * FROM odtProduct where sale_date = '".$Prow[sale_date]."' and sale_enddate = '".$Prow[sale_enddate]."' and parent_code = '".$Prow[parent_code]."' and code != parent_code";
...
if ($hourtype == "00") {    //기본값 종료시간을 지정된 시간으로 표시
        ...
        $STT_DATETIME = $STT_DATE ? "$STT_DATE ".$CHG_HOURPS.":".$CHG_MPS.":00" : "";    //판매시작일(시분초)
...
if($separation == 1){
        $END_DATETIME = $END_DATE ? "$END_DATE ".$Prow[sale_enddateh].":".$Prow[sale_enddatem].":59" : "";    //판매종료일
        $END_DATETIMEH = $END_DATE ? "$END_DATE ".$Prow[sale_enddateh]."" : "";    //판매종료일
        $END_DATETIMEHM = $END_DATE ? "$END_DATE ".$Prow[sale_enddateh].":" . $Prow[sale_enddatem] : "";    //판매종료일
        ...
        $stt_time_data = mktime(substr($STT_DATETIME,11,2),substr($STT_DATETIME,14,2),substr($STT_DATETIME,17,2),
substr($STT_DATETIME,5,2),substr($STT_DATETIME,8,2),substr($STT_DATETIME,0,4));
