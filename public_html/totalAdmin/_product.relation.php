<?PHP
	...
	if(!$code) error_msgPopup_s("잘못된 접근입니다.");
...
<?
unset($res);
$relation_procode_Division = array_unique(array_filter(explode("/",$relation_procode)));
?>
<!-- 리스트영역 -->
	...
	<table class="list_TB" summary="리스트기본">
		<thead>
			<tr>
				...
				<th scope="col" class="colorset"><input type=checkbox name='allcheck' value='Y' onclick="selectAll();" <?=(count($relation_procode_Division) > 0) ? "checked" : ""?>></th>
...
<?PHP
	...
	if($relation_procode) { $s_query_add = "  p.code in ('".implode("','",$relation_procode_Division)."') "; }

	if( !$cateCode && !$pass_parent02 && !$pass_parent01 ) {
		if($s_query_add){
			$que = " select p.* from odtProduct as p where ".$s_query_add." ORDER BY p.inputDate desc ";
			$res = _MQ_assoc($que);
		...
		$que = " 
			select p.* from odtProduct as p
			where (1 " . $s_query . ") ".($s_query_add ? " or (".$s_query_add.") " : "" )."
	...
	if($res){
		foreach($res as $k=>$v) {
			...
			echo "
				<tr class='tr_check'>
					<td>".$_num."</td>
					<td><input id='isRelation' type='checkbox' name='ProCode[]' value='" . $v['code'] . "' " . (in_array($v['code'], $relation_procode_Division) ? "checked" : "") . "></td>
					<td style='text-align:left ; padding-left:5px;'><img src='". replace_image('/upfiles/product/'.($v['prolist_img'] ? $v['prolist_img'] : $v['main_img'])) ."' style='width:100px;'></td>
	...
	} else { echo "<tr><td colspan=6 height='80'>해당분류를 선택하시기 바랍니다.</td></tr>"; }
?>
		</tbody> 
	</table>
</div>
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    function putValue() {
        ...
        $("input[name='ProCode[]']:checked").each(function(){
            app_str = app_str + $(this).val() +"/" ;
        });
        opener.document.frm.p_relation.value = app_str;
        close();
    }
</SCRIPT>
