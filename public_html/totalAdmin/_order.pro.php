<?PHP
	...
	// 돌아갈 페이지 추출
	$return_referer = parse_url($_SERVER['HTTP_REFERER']);
	$return_file_tmp = explode("/",$return_referer['path']);
	$return_file = $return_file_tmp[2];
	$return_to_url = $return_file ? $return_file : "_order.list.php";
	...
	switch ($_mode) {
		...
		case "modify":
			...
			$sque = "
				UPDATE odtOrder SET
					...
					cancel_bank      ='" . rm_str($cancel_bank) . "',
					cancel_bank_account      ='" . rm_str($cancel_bank_account) . "',
			...
			if( $r[canceled] == "N" && $r[paystatus]<>$paystatus && $paystatus == "Y" ) {
				...
				// 제공변수 : $_ordernum
				//include_once("../pages/shop.order.pointadd_pro.php"); 2017-07-25

				// 쿠폰상품은 티켓을 발행한다.
				// 제공변수 : $_ordernum
				include_once("../pages/shop.order.couponadd_pro.php");

				// 상품 재고 차감 및 판매량 증가
				include_once("../pages/shop.order.salecntadd_pro.php");

				// 제휴마케팅 처리 - 무통장입금시 처리
				//include_once("../pages/shop.order.aff_marketing_pro.php");
				...
				if( mailCheck($r[orderemail]) ){
					...
					if( in_array($r[order_type] , array("coupon" , "both"))) {
						$_ordernum = $ordernum;
						$_type = "coupon"; // 쿠폰발송
						include(dirname(__FILE__)."/../pages/shop.order.mail.php"); // 메일 내용 불러오기 ($mailing_content)
		...
		case "cancel":
			...
			if ( $tmp[cnt] > 0 ) {
				...
				if($_trigger == "Y"){ error_loc_msg($return_to_url."?".enc('d',$_PVSC) , "주문을 취소하였습니다."); }
				else { error_loc_msg($return_to_url."?".enc('d',$_PVSC) , "결제 취소요청이 실패하였습니다."); }
				// - 취소처리 ---
		...
		case "force_cancel":
			...
			if($_trigger == "Y"){ error_loc_msg($return_to_url."?".enc('d',$_PVSC) , "주문을 강제취소하였습니다."); }
			else { error_loc_msg($return_to_url."?".enc('d',$_PVSC) , "결제 강제취소요청이 실패하였습니다."); }
			// - 취소처리 ---
		...
		case "mass_cancel":
			...
			foreach($OrderNum as $k=>$v) {
				...
				if ( $tmp[cnt] > 0 ) {
					...
					if($_trigger == "N"){ error_loc_msg($return_to_url."?".enc('d',$_PVSC) , "결제 취소요청이 실패하였습니다."); }
					// - 취소처리 ---
				}
			...
			error_loc_msg($return_to_url."?".enc('d',$_PVSC) , "주문이 취소되었습니다.","top");
			break;
		...
		case "select_wiping":
			...
			if( sizeof($OrderNum) > 0 ) {
				...
				foreach( $res as $k=>$v ){
					...
					_MQ_noreturn("delete from odtRequestReturn where rr_ordernum='". $v[ordernum] ."' ");// 교환반품 정보
				}
			}

			// 교환/반품 내역 삭제 LMH008
			//_MQ_return(" delete from odtRequestReturn where rr_ordernum = '".$v[ordernum]."' ");
		...
		case "select_paystatus" :
			...
			$paystatus = 'Y'; // 입금확인 처리
			$exec_count = 0; // 무통장 처리건 개수

			# -- 무통장입금확인 처리 이므로  $paystatus 는 Y 로 처리한다.
			if( sizeof($OrderNum) > 0 ) {

				foreach($OrderNum as $key=>$ordernum) {

					// 주문발송 상태 변경
					order_status_update($ordernum);

					# -- 주문번호가 일치하고, 결제방식이 무통장일경우
					$r = _MQ("SELECT * FROM odtOrder WHERE ordernum='" . $ordernum . "' AND paymethod = 'B'  ");

					# -- 검색값이 없다면 넘긴다.
					if(count($r) < 1 ) { continue; }

					//- 결제확인 메일 및 문자 발송 --- 취소주문 수정이 아닌경우
					if( $r[canceled] != "Y" && $r[paystatus]<>$paystatus && $paystatus == "Y"){ // NPay  && $r['npay_order'] == 'N' 추가
						...
						// 결제가 확인되었을 경우 - 포인트 쿠폰 - 적용
						// 제공변수 : $_ordernum
						//include("../pages/shop.order.pointadd_pro.php"); // 2017-07-25

						// 쿠폰상품은 티켓을 발행한다.
						// 제공변수 : $_ordernum
						include("../pages/shop.order.couponadd_pro.php");

						// 상품 재고 차감 및 판매량 증가
						include("../pages/shop.order.salecntadd_pro.php");

						// 제휴마케팅 처리
						include("../pages/shop.order.aff_marketing_pro.php");
						...
						if($row_sms[$smskbn][smschk] == "y") {
							...
							# -- 주문자의 휴대폰 번호 출력
							$orderhtel1 = $r['orderhtel1']; $orderhtel2 = $r['orderhtel2']; $orderhtel3 = $r['orderhtel3'];
				...
				} # -- end :: foreach($OrderNum as $key=>$ordernum) {
			} # -- end :: if( sizeof($OrderNum) > 0 ) {

			error_frame_loc_msg("_order.list.php?" . enc('d' , $_PVSC) , "무통장입금확인건 총 [".$exec_count."] 건이 처리되었습니다.");
