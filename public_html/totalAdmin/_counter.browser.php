<?PHP
	...
	$result = _MQ("SELECT SUM(Visit_Num) as COBSV FROM odtCounterOSBrowser WHERE Kinds='B'");
...
?>
				...
				<div class="content_section_inner">
					...
					<table class="list_TB" summary="리스트기본">
						...
						<thead>
							<tr>
								<th scope="col" class="colorset">브라우져</th>
...
<?PHP
	...
	## 최대방문자수 구함
	$max = _MQ(" SELECT ifnull(MAX(Visit_Num),0) as MV FROM odtCounterOSBrowser WHERE Kinds='B' ");
	$max = $max[MV];
	if(!$max) $max = 0;
