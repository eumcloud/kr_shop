...
// 선택 주문상품 - 정산대기로 넘김 처리
function settlement_status(_status){

	var status_string = (_status == "complete" ? "정산완료" : "정산대기" )

	var checkitems = $("input[name='OpUid\\[\\]']:checked");
	if(checkitems.length == 0) {
		alert (status_string + ' 처리할 항목을 선택하시기 바랍니다.');
		return false;
	}


	// JJC - 100개 초과를 넘길 수 없게함.
	if(checkitems.length > 100) {
		alert ('처리할 항목이 100건을 초과할 수 없습니다.');
		return false;
	}


	frm = document.OderAllDelete;
	if(!confirm("선택하신 항목을 "+status_string+"로 처리하시겠습니까?")) {
		return false;
	}

	orgAction = frm.action;
	orgTarget = frm.target;
	frm._mode.value = "settlementstatus_" + _status ;
	frm.action = "_order2.pro.php";
...
function expressSetFun(frm) {
	...
	function express(force) {
		...
		var title2 = "일괄처리할 주문이 100건 이상인경우에는\n\n원활한 쿠폰발송을 위하여 1~2분 간격으로\n\n" + 
					 "100건씩 나누어 처리하시기 바랍니다.\n\n쿠폰번호(배송번호)가 없는 주문은 발행되지않습니다.\n\n" +
					 "배송상품의경우 택배회사가 없다면 발행되지않습니다.\n\n주문을 발행처리 하시겠습니까.?";
		if(force=="Y") {    //재발행일경우
