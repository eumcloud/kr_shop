<?PHP
	...
	if( in_array( $_mode , array('add' , 'modify') ) ){
		...
		$_adm_file_name = _FilePro( "../upfiles/normal" , "_admfile" );
		// -- 등록한 이미지명 ---
	...
	switch( $_mode ){
		...
		case "modify":
			...
			if( in_array($_menu,array('partner')) && $_sendmail == 'Y' ) {
				// - 메일발송 ---
				$_oemail = $_email;
				if( mailCheck($_oemail) ){
					
					$_title = "[".$row_setup[site_name]."] 제휴문의에 관해 답변드립니다.";
					$_title_content = '
		...
		case "delete":
			...
			_FileDel( "../../upfiles/normal" , $r['r_admfile']);
