...
if(!strcmp($form,"sendform")) {
	...
	$arr = array_filter(array_unique(explode("/",$send_list_serial)));
	$arr_send = array();
	...
	if($_FILES['a_file'][size]> 0){
		if($_FILES['a_file'][size] > 60*1024) {

			error_alt("업로드한 파일 크기가 너무 큽니다.\\n60KB 이하로 등록하세요.");
			exit;
		}
		else {
	...
	foreach($arr as $k=>$v){
		...
		if($m_send_type == 'M') { // SMS 분할발송

			$tran_msg = lms_to_sms_array($tran_msg);
			foreach($tran_msg as $kk=>$vv) {

				$arr_send[] = array('receive_num'=> $v , 'send_num'=> $tran_callback , 'msg'=> $vv , 'reserve_time'=>$app_sque, 'title'=> $tran_title, 'image'=>$img_name, 'image_del'=>'Y' );
			}
		}
		else if($m_send_type == 'S') { // SMS 단일발송

			$tran_msg = lms_to_sms_array($tran_msg);
			$arr_send[] = array('receive_num'=> $v , 'send_num'=> $tran_callback , 'msg'=> $tran_msg[0], 'reserve_time'=>$app_sque, 'title'=> $tran_title, 'image'=>$img_name, 'image_del'=>'Y' );
		}
		else { // 일반발송 (90byte 이내: SMS 발송, 90byte 이상: LMS발송)
