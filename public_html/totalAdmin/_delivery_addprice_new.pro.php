<?PHP
	...
	switch( $_mode ){
		...
		case "ajax_form":
			...
			// 폼생성
			$idx = 0;
			echo '<div class="line inner_info"><span class="inner_title">* 추가배송비</span> <ul><li><input type="text" name="_addprice" id="_addprice" class="input_text" value="'. number_format($addprice) .'" style="text-align: right"/>원</li></ul></div>';
			foreach($arr as $k=>$v){
				$_addr = ''; // 주소 이어붇이기
				$_keyval = ''; // 시도+구군 :: 키값으로 사용
				if($k=='jibun') echo '<div class="line inner_info"><span class="inner_title">* 지번주소</span> <ul>';
				if($k=='road') echo '<div class="line inner_info"><span class="inner_title">* 도로명주소</span> <ul>';
				foreach($v as $k2=>$v2){
					$idx++;
					...
					echo '
							<li>
								<input type="text" name="" class="input_text" value="'. $_addr .'" disabled style="width: 300px;" />
								<input type="hidden" name="" id="addr_'. $idx .'" value="'. $_addr .'" readonly />
								<span class="shop_btn_pack" style="float:none;"><input type="button" value="선택추가" class="small white" onclick="insert_addprice('.$idx.')"></span>
							</li>';
				}
				$idx++;
				echo '
					<li>
						<input type="text" name="" id="addr_'. $idx .'" class="input_text" value="'. $_addr .'" style="width: 300px;" />
						<span class="shop_btn_pack" style="float:none;"><input type="button" value="수정추가" class="small blue" onclick="insert_addprice('.$idx.')"></span>
					</li>';
				echo '</ul></div>';
			}
		...
		case "add":
			...
			// 이미 적용된 주소인지 체크
			$chk_cnt = _MQ_result(" select count(*) as cnt from odtDeliveryAddpriceNew where da_addr = '". trim($addr) ."' ");
			if($chk_cnt>0) error_msg("이미 추가배송비가 적용된 주소입니다.");


			// 도서산간지역 추가
			$que = "
				insert into odtDeliveryAddpriceNew set
					da_addr = '". addslashes(trim($addr)) ."'
					,da_price = '". rm_str($addprice) ."' 
					,da_rdate = now()
			";
			_MQ_noreturn($que);
			error_loc_msg("_delivery_addprice_new.list.php?addprice=".$addprice,"정상적으로 추가 되었습니다");
		...
		case "modify":
			...
			// 도서산간지역 추가
			$que = "
				update odtDeliveryAddpriceNew set
					da_addr = '". addslashes(trim($addr)) ."'
					,da_price = '". rm_str($addprice) ."' 
				where da_uid = '". $_uid ."'
			";
		...
		// 추가배송비 삭제
		case "delete":
			_MQ_noreturn("delete from odtDeliveryAddpriceNew where da_uid='$_uid' ");
			error_loc_msg( "_delivery_addprice_new.list.php?" . enc('d' , $_PVSC) ,"정상적으로 삭제되었습니다");
			break;
		...
		case "mass_delete":
			...
			foreach($chk_uid as $k=>$v) {
				_MQ_noreturn("delete from odtDeliveryAddpriceNew where da_uid='". $v ."' ");
			}
		...
		case "mass_modify":
			...
			_MQ_noreturn("update odtDeliveryAddpriceNew set da_price = '". $modify_addprice ."' where da_uid in ('". implode("','",$chk_uid) ."') ");
			error_loc_msg( "_delivery_addprice_new.list.php?" . enc('d' , $_PVSC) ,"추가배송비가 변경되었습니다.");
			break;
		...
		case "ins_excel":
			...
			for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
				...
				// 이미등록된 주소인지 체크
				$chk = _MQ(" select da_uid from odtDeliveryAddpriceNew where da_addr = '". addslashes(trim($r[1])) ."'");
			...
			error_loc_msg("_delivery_addprice_new.list.php" , $msg);
			break;
		...
		case "search_excel": // 검색
			...
			";
			$que = " select * from odtDeliveryAddpriceNew {$s_query} ORDER BY da_addr asc ";
			$res = _MQ_assoc($que);
			foreach($res as $k=>$v) {
		...
		case "create_table": 
				...
				$r = _MQ_assoc(" SHOW TABLES LIKE 'odtDeliveryAddpriceNew'; ");

				if(count($r)>0){
					// 이미추가된 테이블
					//error_loc_msg("_delivery_addprice_new.list.php" , "이미 추가되었습니다.");
				}else{
					// 테이블생성
					$que = "
							CREATE TABLE IF NOT EXISTS `odtDeliveryAddpriceNew` (
					...
					// 기본데이터 추가
					$que = "
						INSERT INTO `odtDeliveryAddpriceNew` (`da_uid`, `da_addr`, `da_price`, `da_rdate`) VALUES
				...
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtSetup` ADD  `s_del_addprice_use` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '추가배송비적용여부' ");
				}
				$add = "s_del_addprice_use_normal";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtSetup` ADD  `s_del_addprice_use_normal` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '일반배송상품을 무료배송비 이상 구매하여 무료배송이 적용된경우 추가배송비 적용여부' ");
				}
				$add = "s_del_addprice_use_unit";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtSetup` ADD  `s_del_addprice_use_unit` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '개별배송상품의 추가배송비 적용여부' ");
				}
				$add = "s_del_addprice_use_free";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtSetup` ADD  `s_del_addprice_use_free` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '무료배송상품의 추가배송비 적용여부' ");
				}

				// -- 입점업체 설정 DB 추가 -----
				$chk = _MQ_assoc(" desc odtMember ");
				//ViewArr($chk);
				$arr = array();
				foreach($chk as $k=>$v){
					$arr[$v['Field']] = "Y";
				...
				//ViewArr($arr);
				$add = "com_del_addprice_use";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtMember` ADD  `com_del_addprice_use` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '추가배송비적용여부' ");
				}
				$add = "com_del_addprice_use_normal";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtMember` ADD  `com_del_addprice_use_normal` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '일반배송상품을 무료배송비 이상 구매하여 무료배송이 적용된경우 추가배송비 적용여부' ");
				}
				$add = "com_del_addprice_use_unit";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtMember` ADD  `com_del_addprice_use_unit` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '개별배송상품의 추가배송비 적용여부' ");
				}
				$add = "com_del_addprice_use_free";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtMember` ADD  `com_del_addprice_use_free` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '무료배송상품의 추가배송비 적용여부' ");
				}

				// -- 입점업체 설정 DB 추가(휴면회원) -----
				$chk = _MQ_assoc(" desc odtMemberSleep ");
				//ViewArr($chk);
				$arr = array();
				foreach($chk as $k=>$v){
					$arr[$v['Field']] = "Y";
				...
				$add = "com_del_addprice_use";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtMemberSleep` ADD  `com_del_addprice_use` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '추가배송비적용여부' ");
				}
				$add = "com_del_addprice_use_normal";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtMemberSleep` ADD  `com_del_addprice_use_normal` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '일반배송상품을 무료배송비 이상 구매하여 무료배송이 적용된경우 추가배송비 적용여부' ");
				}
				$add = "com_del_addprice_use_unit";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtMemberSleep` ADD  `com_del_addprice_use_unit` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '개별배송상품의 추가배송비 적용여부' ");
				}
				$add = "com_del_addprice_use_free";
				if($arr[$add]<>"Y"){
					_MQ_noreturn(" ALTER TABLE  `odtMemberSleep` ADD  `com_del_addprice_use_free` ENUM(  'Y',  'N' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  'N' COMMENT  '무료배송상품의 추가배송비 적용여부' ");
				}

				error_loc_msg("_delivery_addprice_new.list.php" , "추가배송비 DB가 생성되었습니다.");
			break;
