<?php
	...
	$__tPrice_cancel = _MQ_result(" select sum( (op_pprice + op_poptionprice) * op_cnt + op_delivery_price + op_add_delivery_price ) from odtOrderProduct where op_oordernum='" . $ordernum . "' and op_cancel = 'Y' "); // 부분취소할 수 있는 실결제 잔액
	$__tPrice = $opr['tPrice'] > 0 && $opr['tPrice'] <> $__tPrice_cancel ? $opr['tPrice'] - $__tPrice_cancel : $__tPrice; 
	...
	if( $opr['op_cancel_type']=='pg' ) {
		...
		$__availablePoint = $opr['gPrice'] - $opr['gRefundPrice']; // 환불시 사용 가능한 적립금 사용 금액
		$__availablePoint = ($opr['op_cancel']!='N' ? $opr['gPrice'] : $__availablePoint);
	...
	// 포인트 환불
		...
		$__tPriceDiff = $__tPrice + $opr['gPrice'] - $__refundToBe; // 실결제잔액과 고객이 받아야할 총 금액 차이
		$__availablePoint = $opr['tPrice'] + $opr['gPrice'] - $opr['gRefundPrice']; // 환불시 사용 가능한 적립금 사용 금액
