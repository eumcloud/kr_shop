	...
	<div class="form_box_area">
		<table class="form_TB" summary="검색항목">
			...
			<tbody>
				<tr>
					...
					<td class="conts" ><?=_InputSelect("pass_p_view", array("N" , "Y"), $pass_p_view, "", array("숨김" , "노출"), "-선택-")?></td>
					...
					<td class="conts"><?=_InputSelect("pass_customerCode", array_keys($arr_customer), $pass_customerCode, "", array_values($arr_customer), "-공급업체-")?></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="conts" colspan="5">
						<?php
						$r2 = $product_icon;
						$pi_uid_array = $pass_icon;
						foreach($r2 as $k2 => $v2) {
							$checked = @in_array($v2['pi_uid'],$pi_uid_array) ? " checked " : "";
							echo "<input type='checkbox' name='pass_icon[]' value='".$v2['pi_uid']."' ".$checked."><img src='".IMG_DIR_ICON.$v2['pi_img']."' title = '".$v2['pi_title']."'> ";
						}
						?>
					</td>
				</tr>
		...
		<div class="top_btn_area">
			<div class="btn_line_up_center">
				...
				<span class="shop_btn_pack"><span class="blank_3"></span></span>
				<span class="shop_btn_pack"><a href="#none" onclick="$('.excel_upload_field').toggle();return false;" class="medium white" title="일괄업로드">일괄업로드</a></span>
			</div>
		</div>
	</div>
</form>
...
<form name="frm" method="post" action="_product.pro.php">
	...
	<div class="content_section_inner">
		...
		<div class="ctl_btn_area">
			<span class="shop_btn_pack"><a href="#none" class="small gray" title="선택순위수정" onclick="selectSortModify()">선택순위수정</a></span>
		</div>
				...
				foreach($res as $k=>$v) {
					...
					echo "
						<tr>
							...
							<td>
								". ($v['p_view'] == "Y" ? "노출<br>" : "<FONT COLOR='red'>숨김</FONT><br>") ."
								<input type=text name='chk_idx[".$v['code']."]' value='".$v['pro_idx']."' class=input_text style='width:30px;' >
							</td>
							<td>".${_num}."</td>
							<td><img src='". product_thumb_img( $v , "장바구니" , "data" ) ."' style='width:100px;'></td>
