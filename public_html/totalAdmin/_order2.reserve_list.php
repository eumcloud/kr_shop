...
#LDD018
// 페이지 표시
$app_current_link = "/totalAdmin/_order2.reserve_list.php";
...
if($isExcel=="Y") {
    ...
    // -- 운송정보가 들어있는 테이블을 조회 ---
	$arrExcel = array();
    $que = "select * from odtExpressTmpTable where partnerCode = 'admin'";
    $res = _MQ_assoc($que);
...
$s_query .=" and op.op_delivstatus='N' ";
$s_query .= " and op.op_orderproduct_type='product' ";
...
if($pass_settlementstatus) { $s_query .= " and op_settlementstatus = '{$pass_settlementstatus}' "; } // LDD007
	...
	<table class="form_TB" summary="검색항목">
		...
		<tbody>
			<tr>
				...
				<td class="conts">
					<B><?=number_format($app_tPrice);?></B>원
			...
			<tr>
				...
				<td class="conts" colspan=3>
					<table class="list_TB">
						...
						<tbody>
							<tr>
								<td style="color:#ff0000; font-weight:bold;"><?php echo number_format(reserve_order(1)); ?>건</td>
								<td><?php echo number_format(reserve_order(3)); ?>건</td>
								<td><?php echo number_format(reserve_order(7)); ?>건</td>
								<td><?php echo number_format(reserve_order(15)); ?>건</td>
							</tr>
						</tbody>
					</table>
				</td>
...
<form name=OderAllDelete method=post action="_order2.pro.php" target="common_frame" >
	...
	<div class="content_section_inner">
		...
		<div class="top_btn_area">
			...
			<?php if($delivstatus == "no" && $ordertype == "coupon") { // (쿠폰)발급대기관리 ?>
			...
			<?php if($delivstatus == "yes" && $ordertype == "product") { // (배송)발송완료관리 ?>
			<span class="shop_btn_pack"><a href="#none" onclick="express('Y')" id="batchexpress" class="small white" title="배송정보재발송" >배송정보재발송</a></span>
			<span class="shop_btn_pack"><span class="blank_3"></span></span>
			<?php } ?>

			<?php if($delivstatus == "yes") { // 발송완료관리 ?>
			<span class="shop_btn_pack"><a href="#none" onclick="settlement_status('ready');" class="small white" title="선택정산대기" >선택정산대기</a></span>
			<span class="shop_btn_pack"><span class="blank_3"></span></span>
			<?php } ?>
		...
		<table class="list_TB" summary="리스트기본">
			...
			<tbody>
				...
				if(sizeof($res)==0) {
					...
					echo "<tr><td colspan='9' height='100' style='text-align:center;'><font color='darkorange'>주문 내역이 없습니다.</font></td></tr>";
				}
				...
				foreach($res as $sk=>$sv){
					...
					if($sv['op_orderproduct_type'] == "product") {
						...
						// 배송일경우 수정버튼
						$_mod .="<span class='shop_btn_pack'>&nbsp;<input type=button class='input_small gray ex_modify' data-uid='".$sv['op_uid']."' value='".$ex_modify_text."'/></span>";
					...
					// --- *** 쿠폰일 경우 *** ---
						...
						if(sizeof($coupon_assoc) > 0 ) {
							...
							foreach($coupon_assoc as $coupon_key => $coupon_row) {
								...
								$tmp_content .= "
									<li style='clear:both;display:inline; float:left; padding-top:3px;'>
										<input type='hidden' name='OrderNumValue[]' value='".$sv['ordernum']."'>
										<input type='hidden' name='op_uid[]' value='". $sv['op_uid'] ."'>
										<input type='hidden' name='opc_uid[]' value='".$coupon_row['opc_uid']."'>
										<input type=text warn1='" . $warning1 . "' warn2='" . $warning2 . "' qty='" . $sv['op_cnt'] . "' ordernum='" . $sv['ordernum'] . "' op_uid='". $sv['op_uid'] ."'  name='expressnum[]' ordertype='".$sv['op_orderproduct_type']."' class=input_textSmall style='width:125px' value='" . $coupon_row['opc_expressnum'] . "' readonly>
									</li>
							...
							for($i=0;$i<$sv['op_cnt'];$i++) {
								...
								$tmp_content .= "
									<li style='clear:both;display:inline; float:left; padding-top:3px;'>
										...
										<input type=text warn1='" . $warning1 . "' warn2='" . $warning2 . "' qty='" . $sv['op_cnt'] . "' ordernum='" . $sv['ordernum'] . "' op_uid='". $sv['op_uid'] ."' name='expressnum[]' ordertype='".$sv['op_orderproduct_type']."' class=input_textSmall style='width:125px' value='' readonly>
									</li>
									<li style='display:inline; float:right; padding-top:3px;'><span class='shop_state_pack'>" . $usestring . "</span></li>
								";
							}
					...
					echo "
						<tr>
							...
							<td >
								...
								". (substr($sv['paydate'],0,10) == "0000-00-00" ? "-" : date('y.m.d H:i',strtotime($sv['paydate']))) ."<!-- 결제일 --> <br>
							</td>
