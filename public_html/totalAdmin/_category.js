...
function f_add(catedepth,serialnum,framename)
{
    ...
    window.open('_category.pro.php?status=menu_add&catedepth=' + catedepth + '&parent_catecode=' + parent_catecode + '&serialnum=' + serialnum + '&framename=' + framename,'메뉴추가','width=500,height=400,toolbar=no,scrollbars=yes,top=0,left=0');
    return rtn_cd;
}
...
{
        var check_trigger = document.getElementsByName('check_trigger')[0].value;
        if(check_trigger==1){
