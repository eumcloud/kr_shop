<?PHP
	...
	if(rm_str($row_setup['s_deny_tel']) == rm_str($refusal_num) ) {
		...
		// 수신거부를 할 회원 체크
		$mr_cnt = _MQ("select count(*) as cnt from odtMember where concat(htel1 ,  htel2 , htel3) = '". rm_str($hp) ."' ");

		if( $mr_cnt['cnt'] == 1 ) {
			_MQ_noreturn(" update odtMember set sms = 'N' , m_opt_date = now() where concat(htel1 ,  htel2 , htel3) = '". rm_str($hp) ."' ");
			...
			// $hp 변수를 이용하여 회원정보 추출
			// $_trigger = "OK" 여야 진행가능
			$_change_alert_file_name = $_SERVER["DOCUMENT_ROOT"] . "/include/addons/changeAlert/changeAlert.mail.contents.080deny.php";
