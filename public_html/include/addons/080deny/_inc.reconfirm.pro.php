	...
	switch( $_mode ){
        ...
        case "sms_chk_again_select":
            ...
            if($_action == 'check'){ // 체크라면
                ...
                $que = " select count(*) as cnt from odtMember where userType='B' and sms = 'N' and id in ('". implode("','" , array_values($chk_id)) ."') ";
                $chkr = _MQ($que);
                ...
                if($_type == 'deny'){ // 제외발송 => 제외된 항목의 id 를 보내준다.
                    $deny_arr = array();
                    $que = " select id from odtMember where userType='B' and sms = 'N' and id in ('". implode("','" , array_values($chk_id)) ."') ";
                    $chkr = _MQ_assoc($que);
                    foreach($chkr as $k=>$v){
                        $deny_arr[] = $v['id']; 
                    }
        ...
        case "sms_chk_again_search":
            ...
            if($_action == 'check'){ // 체크라면
                $que = " select count(*) as cnt from odtMember " . enc('d' ,  $_search_que ) . " and sms = 'N' ";
                $chkr = _MQ($que);
                echo json_encode(array('rst'=>'success','deny_cnt'=>$chkr['cnt']));
                exit;
            }else{
                ...
                if($_type == 'deny'){ // 제외발송 => 제외된 항목의 id 를 보내준다.
                    ...
                    if(preg_match("/(and sms = \'Y\')/",$squery) == true){
                    ...
                    $que = " select count(*) AS cnt from odtMember " .$squery  ." ";
