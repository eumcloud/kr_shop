				...
				<?php
					...
					if($row_setup['s_2year_opt_use'] == "Y") {
						// JJC : 수정 : 2021-05-17
						//$mr_cnt = _MQ(" select count(*) as cnt from odt2yearOptLog where ol_status='N'  "); 
						$mr_cnt = _MQ(" select count(*) as cnt from odt2yearOptLog INNER join odtMember on (id = ol_mid and userType='B') where ol_status='N'  "); 
						if( $mr_cnt['cnt'] > 0 ) :
				?>
