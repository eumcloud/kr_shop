<?php
	...
	if($MgtKey && $row_setup['TAX_CERTKEY'] && $CorpNum && $ID && $PWD) {
		...
		if ($Result < 0){
			...
			echo "<div style='float:left;margin-left:5px;'><span class='shop_btn_pack'><input type='button' onclick=\"open_window('tax_print', '" . $Result . "', 100, 100, 1000, 700, '', '', '', '', '');\" value='세금계산서 정보' class='input_small gray'></span></div>";
		}
	}
