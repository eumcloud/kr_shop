<?php
	...
	if( $id ) {
		...
		// - 메일발송 ---
		if( mailCheck($email) && ( $row_member['mailling'] <> $_mailling || $row_member['sms'] <> $_sms ) && ( $_mailling || $_sms ) ){

			$mail_string = '';
			// 메일링 수신여부 체크
			if($row_member['mailling'] <> $_mailling && $_mailling ) {
				$mail_string .= '
			...
			$_title = "[".$row_setup[site_name]."] 정보수정으로 수신동의 상태가 변경되었습니다.";
			$_title_content = '
				<strong style="font-family:\'나눔고딕\',\'돋움\'; color:#000; font-weight:600">' . $row_member['name'] .'님!</strong><br />
