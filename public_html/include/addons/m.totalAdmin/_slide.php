...
?>
		...
		<div class="menu_box">
			<ul>
				<!-- 2뎁스 메뉴없는경우 only -->
				<?if( in_array("/totalAdmin/_product.list.php" , array_keys($arr_save_link)) ) : ?>
				<li class="only">
				...
				<?if( in_array("/totalAdmin/_order.list.php" , array_keys($arr_save_link)) ) : ?>
				<li class="open">
				...
				<!-- 2뎁스 메뉴없는경우 only -->
				<?if( in_array("/totalAdmin/_request.list.php?pass_menu=request" , array_keys($arr_save_link)) ) : ?>
				<li class="only">
					<a href="_request.list.php?pass_menu=request" class="depth1">1:1문의</a>
					<a href="_request.list.php?pass_menu=request" class="btn_icon" title="메뉴열기"><span class="shape"></span></a>
				</li>
				<?endif;?>
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function sitemap_ctl(){
		...
		if(type=="block"){
			...
			// 사이트맵 슬라이드
			$(".sitemap").animate({"left":"-100%"},300,function(){$(".sitemap").hide();});
			// 백그라운드
