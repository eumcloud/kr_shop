<?PHP
	...
	switch( $_mode ){
		...
		case "auth": 
			...
			if( date("Y-m-d") <> $ex[0] ) { error_loc_msg("/?pn=member.login.form" , "재 인증 받으시기 바랍니다."); }
		...
		case "send": 
			...
			if( mailCheck($email) ){
				...
				$_AUTH_URL = "http://" . $_SERVER['HTTP_HOST'] . "/pages/member.sleep.pro.php?_mode=auth&auth=" . onedaynet_encode( $_app_auth ) ;

				include_once(dirname(__FILE__)."/mail.contents.sleep_send.php"); // 메일 내용 불러오기 ($mailing_content)
