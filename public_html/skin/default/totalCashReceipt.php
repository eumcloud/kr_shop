<?
	...
	// 2016-09-09 추가 ::: JJC_TAX - 현금영수증 부가 적용 오류 수정
    $app_vat = ceil(($amount) / 11);// 부가세 계산
	//공급가 ==> $amount - $app_vat ;;

	if($order[taxorder]=='Y') { // 현금영수증 신청이 된 상태면 PG사 연동

		/******* LG U+ ***************/
		if($row_setup[P_KBN]=='L') {
		...
		if($row_setup[P_KBN]=='I') {
			...
			if($method=='AUTH') { // 현금영수증 발행 요청
				...
				$inipay->SetField("sup_price"       , ($amount - $app_vat));                                    // 공급가액 -- 2016-09-09 수정
				$inipay->SetField("tax"             , $app_vat);                                        // 부가세 -- 2016-09-09 수정
				$inipay->SetField("srvc_price"		,0);										// 봉사료
				$inipay->SetField("buyername"		,$order[ordername]);							// 구매자 성명
				$inipay->SetField("buyeremail"		,$order[orderemail]);							// 구매자 이메일 주소
				$inipay->SetField("buyertel"		,$order[orderhtel1].'-'.$order[orderhtel2].'-'.$order[orderhtel3]);							// 구매자 전화번호
...
****************************************************************/
		...
		if($row_setup[P_KBN]=='K') {
			...
			if ( $method == "AUTH" ) {
				...
				//$current = _MQ("select * from odtOrderOnlinelog where ool_ordernum = '$ordernum' order by ool_uid desc limit 1");


				// 현금영수증 정보
	            $rcpt_data_set .= $c_PayPlus->mf_set_data_us( "user_type",      'PGNW' ); // V5 = PGNW , V6 = MT31T08661 // 테스트아이디: V5 = T0000 , V6 = MT31T00875
	            $rcpt_data_set .= $c_PayPlus->mf_set_data_us( "trad_time",      date('YmdHis',strtotime($order[orderdate]))        ); // 원거래시각
	            ...
	            $rcpt_data_set .= $c_PayPlus->mf_set_data_us( "amt_sup",        ($amount - $app_vat)          ); // 공급가액 -- 2016-09-09 수정
				$rcpt_data_set .= $c_PayPlus->mf_set_data_us( "amt_svc",        '0'          ); // 봉사료
				$rcpt_data_set .= $c_PayPlus->mf_set_data_us( "amt_tax",        $app_vat          ); // 부가가치세 -- 2016-09-09 수정
	            ...
	            $c_PayPlus->mf_set_ordr_data( "buyr_name",  $order[ordername] );
	            $c_PayPlus->mf_set_ordr_data( "buyr_tel1",  $order[orderhtel1].'-'.$order[orderhtel2].'-'.$order[orderhtel2] );
	            $c_PayPlus->mf_set_ordr_data( "buyr_mail",  $order[orderemail] );
				...
				$current = _MQ("select * from odtOrderCashlog where ocs_ordernum = '$ordernum' and ocs_method = 'AUTH' order by ocs_uid desc limit 1");
				...
				if($c_PayPlus->m_res_cd == "0000") {
					...
					_MQ_noreturn("
						insert into odtOrderCashlog (
							...
							'$ordernum', '$order[orderid]', now(), '$cash_no', '$receipt_no', '$app_time', '$res_desc', '$method', '', '$amount', '$order[paymethod]', ''
						)
					");
					$return = $method;
				} else { $return = 'FAIL'; }
		...
		/******* 올더게이트 ***************/
		if($row_setup[P_KBN]=='A') {
			// 현금영수증 발행 기능 없음
		}

		/******* 엠엔뱅크 ***************/
		if($row_setup[P_KBN]=='M') {
			// 현금영수증 발행 기능 없음
		}
    
	    /******* 다우페이 ***************/
	    if($row_setup[P_KBN]=='D') {
	        // 현금영수증 발행 기능 없음
	    }

		/******* 빌게이트 ***************/
		if($row_setup[P_KBN]=='B') {
			//---------------------------------------
			// Include Class(don't modify)
			//---------------------------------------

			$SERVICE_ID = $row_setup[P_ID] ;   //테스트서버 : glx_api
			...
			$ORDER_ID = $ordernum;
			$ORDER_DATE = date('YmdHis',strtotime($order[orderdate])); // 주문일자
			$DEAL_DATE = $method=='AUTH'?date('YmdHis',strtotime($order[paydate])):date('YmdHis'); // 거래일자
			$DEAL_AMOUNT = $amount - $app_vat ; // 금액 -- 2016-09-09 수정
			$VAT = $app_vat ; // 부가세 -- 2016-09-09 수정
			$SERVICE_CHARGE = 0; // 봉사료
			$USING_TYPE = '0'; // 0: 소득공제용 , 1: 지출증빙용
			$DEAL_TYPE = $method=='AUTH'?'0':'1'; // 0: 승인요청 , 1: 취소요청
			$IDENTIFIER = $num; // 현금영수증 발행 키 값 (휴대전화번호)
			$IDENTIFIER_TYPE = '20'; // 10: 주민번호발급 , 20: 휴대전화번호발급 , 30: 사업자번호발급 , 40: 카드번호발급
			$ITEM_NAME = cutstr($product,20); // 상품명
			$TRANSACTION_ID = $tid;
			$CANCEL_REASON_TYPE = '3'; // 취소사유 = 1: 거래취소 , 2: 오류발급 , 3: 기타
			...
			$reqMsg->setServiceCode($svcCode->CASH_RECEIPT); //서비스코드
			if($method=='AUTH') { 
				$reqMsg->setCommand($cmd->CASHRECEIPT_REQUEST); //발급 요청 Command
			} else {
				$reqMsg->setCommand($cmd->CANCEL_REQUEST); //발급 취소 요청 Command
			}
			...
			//---------------------------------------
			if($method=='AUTH') {
				if($DEAL_DATE != NULL) //거래일시
				        $reqMsg->put($tag->DEAL_DATE, $DEAL_DATE);
				if($DEAL_AMOUNT != NULL) //신고금액
				        $reqMsg->put($tag->DEAL_AMOUNT, $DEAL_AMOUNT);
				if($VAT != NULL) //부가세
				        $reqMsg->put($tag->VAT, $VAT);
				if($SERVICE_CHARGE != NULL) //봉사료
				        $reqMsg->put($tag->SERVICE_CHARGE, $SERVICE_CHARGE);
				if($USING_TYPE != NULL) //거래자 구분
				        $reqMsg->put($tag->USING_TYPE, $USING_TYPE);
				if($DEAL_TYPE != NULL) //거래구분
				        $reqMsg->put($tag->DEAL_TYPE, $DEAL_TYPE);
				if($IDENTIFIER != NULL) //신분확인
				        $reqMsg->put($tag->IDENTIFIER, $IDENTIFIER);
				if($IDENTIFIER_TYPE != NULL) //현금영수증 발행 타입
				        $reqMsg->put($tag->IDENTIFIER_TYPE, $IDENTIFIER_TYPE);
				if($BUSINESS_TYPE != NULL) //사업자 유형
				        $reqMsg->put($tag->BUSINESS_TYPE, $BUSINESS_TYPE);
				if($ORIGINAL_AUTH_NUMBER != NULL) //승인거래 승인번호
				        $reqMsg->put($tag->ORIGINAL_AUTH_NUMBER, $ORIGINAL_AUTH_NUMBER);
				if($ORIGINAL_DEAL_DATE != NULL) //승인거래 거래일자
				        $reqMsg->put($tag->ORIGINAL_DEAL_DATE, $ORIGINAL_DEAL_DATE);
				if($CANCEL_REASON_TYPE != NULL) //취소 사유
				        $reqMsg->put($tag->CANCEL_REASON_TYPE, $CANCEL_REASON_TYPE);
				if($ITEM_NAME != NULL) //구매 상품명
				        $reqMsg->put($tag->ITEM_NAME, $ITEM_NAME);
			} else {
				if($TRANSACTION_ID != NULL) //갤럭시아 거래 번호
				        $reqMsg->put($tag->TRANSACTION_ID, $TRANSACTION_ID);
				if($DEAL_DATE != NULL) //취소 거래일시
				        $reqMsg->put($tag->DEAL_DATE, $DEAL_DATE);
				if($CANCEL_REASON_TYPE != NULL) //취소사유
				        $reqMsg->put($tag->CANCEL_REASON_TYPE, $CANCEL_REASON_TYPE);
			}
			...
			$broker->invoke($svcCode->CASH_RECEIPT); //응답 요청
			...
			if(!strcmp($resMsg->get($tag->RESPONSE_CODE), "0000")) {
			  ...
			  if($method=='AUTH') { $TRANSACTION_ID = $resMsg->get($tag->TRANSACTION_ID); }
			...
			if($isSuccess == true) {
				...
				// 2. 결제 성공 페이지 호출
				//---------------------------------------
				$ocs_ordernum = $ORDER_ID;
				$ocs_tid = $TRANSACTION_ID;
				$ocs_cashnum = $AUTH_NUMBER;
				$ocs_respdate = date('Y-m-d H:i:s');
				$ocs_seqno = '';
				$ocs_msg = $RESPONSE_MESSAGE;
				$ocs_type = $order[paymethod];

				_MQ_noreturn("
					insert into odtOrderCashlog (
						ocs_ordernum, ocs_member, ocs_date, ocs_tid, ocs_cashnum, ocs_respdate, ocs_msg, ocs_method, ocs_cardnum, ocs_amount, ocs_type, ocs_seqno
					) values (
						'$ordernum', '$order[orderid]', now(), '$ocs_tid', '$ocs_cashnum', now(), '$ocs_msg', '$method', '', '$amount', '$order[paymethod]', ''
					)
				");
				$return = $method=='AUTH'?'AUTH':'CANCEL';
			}
			else {
				//---------------------------------------
			...
			// - 주문결제기록 저장 ---
			$app_oc_content = ""; // 주문결제기록 정보 이어 붙이기
			$keys = array('AUTH_NUMBER','TRANSACTION_ID','RESPONSE_CODE','RESPONSE_MESSAGE','DETAIL_RESPONSE_CODE','DETAIL_RESPONSE_MESSAGE');
			foreach($keys as $name) { $app_oc_content .= $name . "||" .iconv("euc-kr","utf-8",$resMsg->get($tag->$name)) . "§§" ; }
			_MQ_noreturn("
				insert odtOrderCardlog set
				 oc_oordernum = '".$ordernum."'
				,oc_tid = '". $ocs_tid ."'
