...
<div class="layout_fix">
	...
	<div class="cm_mypage_action">
		...
		<div class="change_point">
			...
			<ul>
				<li><input type="text" name="_point" class="input_design" placeholder="1,000점 이상" /></li>
				<li><span class="button_pack"><a href="#none" onclick="pointChange();return false;" class="btn_md_black">적립금으로 전환하기</a></span></li>
			</ul>
			</form>
		</div>
	...
	<div class="cm_mypage_list list_point">
		...
		<table summary="쿠폰목록">
			...
			<? foreach($coupon_assoc as $key => $row) { ?>
				<tr>
					<td class="date"><?=date('Y-m-d H:i:s',strtotime($row['acRegidate']))?></td>
					<td class="title"><?=$row['acTitle']?></td>
					<td class="price"><?=number_format($row['acPoint'])?>점</td>
				</tr>
			<? } ?>
			</tbody> 
		</table>
