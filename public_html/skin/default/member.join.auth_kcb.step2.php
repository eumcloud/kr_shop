<?PHP
	...
	// --------------- 환경설정 : 2013-07-31 정준철 -------------
	$onedaynet_kcb_id = $row_setup[s_join_auth_kcb_code];// KCB 회원사 코드 - 환경설정에 포함
	...
	$clientDomain = ($row_company['name'] ? urlencode($row_company['name']) : $_SERVER['HTTP_HOST'] ); // 회원사 도메인. (휴대폰인증번호 발송시 제휴사명에 노출)
	...
	$returnUrl = "http://".$_SERVER["HTTP_HOST"]."/pages/member.join.auth.step3.php";// 본인인증 완료후 리턴될 URL (도메인 포함 full path)
