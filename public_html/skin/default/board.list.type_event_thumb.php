<div class="cm_board_event_thumb">
<? if( count($res)==0 ) { ?>
	<!-- 내용없을경우 모두공통 -->
	...
	?>
		...
		<li>
			...
			<div class="posting">
				<!-- 유동적 x 150 -->
				<span class="thumb">
				<? if( $v['b_thumb'] && @filesize($_dir."/".$v['b_thumb']) > 0 ) { ?>
				<img src="<?=IMG_DIR_BOARD.$v['b_thumb']?>" alt=""/>
				<? } ?>
				</span>
				<div class="conts">
					<dl>
						<dt><?=cutstr($app_title,40)?></dt>
						<dd><?=cutstr(stripslashes(strip_tags(str_replace('&nbsp;', ' ', htmlspecialchars_decode($v['b_content'])))),45)?></dd>
					</dl>
				</div>
