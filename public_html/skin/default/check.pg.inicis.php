...
$card_fail_order = _MQ_assoc(" select ordernum,orderdate from odtOrder where paymethod in ('C', 'L') and paystatus='N' and canceled='N' and orderdate >= '". date("Y-m-d", strtotime("-1 days"))." 00:00:00' ");
foreach( $card_fail_order as $k => $v){

	$card_log = _MQ(" select oc_tid from odtOrderCardlog where oc_oordernum ='".$v['ordernum']."' and oc_tid != '' and (oc_content like '%resultCode||0000§§%' or oc_content like '%m_resultCode||00§§%') ");
