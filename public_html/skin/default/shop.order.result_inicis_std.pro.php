  ...
  $ool_bank_name_array = array(
	...
	$inipay = new INIStdPayUtil();
 	...
 	try {
	    ...
	    $signKey 			=  $row_setup[P_SKEY]; 			// 가맹점에 제공된 키(이니라이트키) (가맹점 수정후 고정) !!!절대!! 전문 데이터로 설정금지
	      ...
	      $access_result = false; // 초기화
				...
				$que = "
					insert odtOrderCardlog set
						...
						,oc_rdate = now();
				";
				// JJC : 망 취소 수정 : 2021-03-15
				if(!preg_match('/중복/i' , $app_oc_content)) {
					_MQ_noreturn($que);
	      ...
	      if($access_result <> true){ // 보안코드값 없이 왔다면
	     	...
	     	*/
	     	...
	     	if (isset($resultMap["payMethod"]) && strcmp("VBank", $resultMap["payMethod"]) == 0) { //가상계좌 => 무통장
					...
					$depositor = $order[ordername]?$order[ordername]:@(in_array($resultMap["VACT_InputName"] , $resultMap) ? $resultMap["VACT_InputName"] : "null" );
					...
					$tno = @(in_array($resultMap["tid"] , $resultMap) ? $resultMap["tid"] : "null" );
			...
			// --- JJC : 망 취소 수정 : 2021-03-15 ---
					...
					$oc_res_cnt = _MQ(" select count(*) as cnt from odtOrderCardlog where oc_oordernum = '".$ordernum."' and oc_tid = '". $resultMap['tid']."' and (oc_content like '%resultCode||0000§§%' or oc_content like '%m_resultCode||00§§%') ");
					if($oc_res_cnt['cnt'] == 1 ) {

						// 결제 실패기록 삭제
						if($insert_oc_uid){_MQ_noreturn("delete from odtOrderCardlog where oc_uid='". $insert_oc_uid ."' ");}
					...
					// 결제실패 처리
						...
						_MQ_noreturn("update odtOrder set orderstatus_step='결제실패' , orderstep='fail' , ordersau = '". addslashes($resultMap["resultMsg"]) ."' where ordernum='". $ordernum ."' ");
						error_loc_msg("/?pn=shop.order.result" , '결제에 실패하였습니다. 다시한번 확인 바랍니다. ['.$_REQUEST["resultCode"].']' ,"top");
			...
			// --- JJC : 망 취소 수정 : 2021-03-15 ---
                    ...
                    if ($httpUtil->processHTTP($netCancel, $authMap)) {
                        ...
                        $netcancelResultString = $httpUtil->body;
						 ...
						 error_loc_msg("/?pn=shop.order.result" , $s ,"top");  
	 		 ...
	 		 error_loc_msg("/?pn=shop.order.result" , '결제요청에 실패하였습니다. ['.$_REQUEST["resultCode"].']' ,"top");
	 	}
	}catch (Exception $e) {
            $s = $e->getMessage() . ' (오류코드:' . $e->getCode() . ')';
            error_loc_msg("/?pn=shop.order.result" , $s ,"top");
