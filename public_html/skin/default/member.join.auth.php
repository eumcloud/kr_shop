<form name="form_auth" class="js_join_auth" autocomplete="off">
	...
	-->
	<!-- 사이트코드 -->
	<input type="hidden" name="site_cd" value="<?php echo $row_setup['s_join_auth_kcb_code']; ?>" />
	<!-- Ret_URL : 인증결과 리턴 페이지 ( 가맹점 URL 로 설정해 주셔야 합니다. ) -->
	<input type="hidden" name="Ret_URL" value="<?php echo ($_SERVER['HTTPS'] == 'on' || $_SERVER['HTTPS'] == 1 ? 'https://' : 'http://'). $_SERVER['HTTP_HOST']; ?>/pages/member.join.auth.step3.php" />
...
<script type="text/javascript">
	...
	function auth_type_check()
	{
		...
		else
		{
			...
			auth_form.action = "/pages/member.join.auth.step2.php"; // 인증창 호출 및 결과값 리턴 페이지 주소
			auth_form.submit();
	...
	$(document).ready(function(){
		$('.js_auth_before').on('click', function(){
			alert('휴대폰 본인인증을 통해 입력(변경)가능합니다.');
			return false;
		});
	});
