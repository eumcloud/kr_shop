<?
	// 거래번호
	if( $ordr[paymethod] == 'V' ) {
		$tmp = _MQ(" select ool_tid from odtOrderOnlinelog where ool_ordernum = '".$ordr[ordernum]."' and ool_type = 'I' order by ool_date desc limit 1 ");
		$LGD_TID = $tmp[ool_tid];
	} else {
	...
	if( $ordr[paymethod] == 'V' ) {
		$cancel_bank = $ordr['cancel_bank'];
		$cancel_bank_account = $ordr['cancel_bank_account'];
