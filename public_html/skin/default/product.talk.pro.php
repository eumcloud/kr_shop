		...
		case "view":
			...
			foreach( $res as $k=>$v ){
				...
				foreach($reply_r as $k2 => $v2) {
					...
					$talk_btn2 = $v2['ttID'] == get_userid() ? "<a href='#none' onclick=\"talk_del('".$v2['ttNo']."');return false;\" title='댓글삭제' class='btn_delete'></a>" : NULL;
				...
				echo "
					<li id='".$v['ttNo']."'>
						<div class='post_box'>
							...
							<a href='#none' onclick=\"talk_show('".$v['ttNo']."');return false;\" class='title'>".cutstr(strip_tags($talk_content),40)."</a>
							<span class='title_icon'>".(time()-strtotime($v['ttRegidate'])<86400*2?"<img src='/pages/images/cm_images/board_ic_new.gif' alt='새글'/>":"")."</span>
							<span class='button_pack'>
								".$talk_show.$talk_close.$talk_btn."
							</span>
							<span class='writer'>
...
<div class='cm_paginate'>
	<span class="lineup">
		...
		<span class="nextprev">
			...
			<span class="btn <?=($listpg != $first)?'click':''?>">
				<? if($listpg == $first){ ?>
				<span title="처음페이지" class="no"><span class="icon ic_first"></span></span>
				<? } else { ?>
				<a href="#none" onclick="talk_view(<?=$first?>);return false;" title="처음페이지" class="ok"><span class="icon ic_first"></span></a>
				<? } ?>
			</span>
			<span class="btn <?=($listpg != $prev)?'click':''?>">
				<? if($listpg == $prev){ ?>
				<span title="이전페이지" class="no"><span class="icon ic_prev"></span></span>
				<? } else { ?>
				<a href="#none" onclick="talk_view(<?=$prev?>);return false;" title="이전페이지" class="ok"><span class="icon ic_prev"></span></a>
				<? } ?>
			</span>
		</span>
		...
		<?
			for($ii=$for_start_num;$ii<=$for_end_num;$ii++) {
				if($ii != $listpg) { echo "<a href='#none' onclick=\"talk_view(".$ii.");return false;\">$ii</a>"; }
				else { echo "<a href='#none' onclick='return false;' class='hit'>$ii</a>"; }
			}
		?>
		</span>
		...
		<span class="nextprev">
			<!-- 클릭할페이지가 생기면 클래스명추가 click -->
			<span class="btn <?=($listpg != $next)?'click':''?>">
				<? if($listpg == $next){ ?>
				<span title="다음페이지" class="no"><span class="icon ic_next"></span></span>
				<? } else { ?>
				<a href="#none" onclick="talk_view(<?=$next?>);return false;" title="다음페이지" class="ok"><span class="icon ic_next"></span></a>
				<? } ?>
			</span>
			<span class="btn <?=($listpg != $last)?'click':''?>">
				<? if($listpg == $last){ ?>
				<span title="마지막페이지" class="no"><span class="icon ic_last"></span></span>
				<? } else { ?>
				<a href="#none" onclick="talk_view(<?=$last?>);return false;" title="마지막페이지" class="ok"><span class="icon ic_last"></span></a>
				<? } ?>
			</span>
		</span>
	</span>
