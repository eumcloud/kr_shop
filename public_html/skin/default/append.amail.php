...
*/
    ...
    $setup_user_id = 'new_'.$row_setup[amail_id];

    // 사전설정
    if( !in_array($mode , array("groupftp","grouplist" , "groupchoice"))) {
        error_alt("잘못된 접근입니다.1");
    }

    if( $user_id <> $setup_user_id ) {
        error_alt("잘못된 접근입니다.2 $user_id - $setup_user_id");
    }


    if( !in_Array($_SERVER["REMOTE_ADDR"] , array("175.209.46.91" , "119.207.76.26" , "175.209.46.100")) ) {//지정 아이피 확인
        error_alt("지정한 아이피가 아닙니다. $ip");
    }


//    if( !($group_code && $mode) ) {
//        error_alt("요청 parameter가 부족합니다.");
//    }
...
*/
    ...
    switch( $group_code ){
        case "T": // 테스트
            $que = "SELECT id, email, name FROM odtMember WHERE  id='admin'";
            $que2 = "SELECT count(*) as cnt FROM odtMember WHERE  id='admin'";
            break;
        case "all": // 전체회원
            $que = "SELECT id, email, name FROM odtMember WHERE  secession='N' and email !='' and userType = 'B'";
            $que2 = "SELECT count(*) as cnt FROM odtMember WHERE  secession='N' and email !='' and userType = 'B'";
            break;
        case "M": // 메일링회원
            $que = "SELECT id, email, name FROM odtMember WHERE  secession='N' and email !='' AND mailling = 'Y' and userType = 'B'";
            $que2 = "SELECT count(*) as cnt FROM odtMember WHERE  secession='N' and email !='' AND mailling = 'Y' and userType = 'B'";
            break;
        case "S": // 구독회원
            $que = "SELECT ft_email as id , ft_email as email , ft_email as name FROM feedTable WHERE ft_email !='' and ft_emailsend='Y' ";
            $que2 = "SELECT count(*) as cnt FROM feedTable WHERE ft_email !='' and ft_emailsend='Y'";
            break;
    }
