<?PHP
	...
	switch( $_mode ){
		...
		case "cancel":
			...
			// - 취소처리 ---
			$_ordernum = $ordernum;
			$_applytype = "member";// 실행타입 : admin , member ::: member 일 경우 반드시 get_userid() 있어야 함
			...
			if($_trigger == "Y"){ error_frame_loc_msg("/?pn=mypage.order.list&" . enc('d' , $_PVSC) , "주문을 취소하였습니다."); }
			else { error_frame_loc_msg("/?pn=mypage.order.list&" . enc('d' , $_PVSC) , "결제 취소요청이 실패하였습니다."); }
			// - 취소처리 ---
		...
		case "complete":
			...
			if(is_login())
				$sub_que = " o.orderid='".get_userid()."' and ";
			else
				$sub_que = " o.orderid='".$orderid."' and o.o_memtype='N' and ";
			...
			$que = " select op.*, p.partnerCode from odtOrder as o
								...
								o.canceled ='N' and
								o.o_paystatus ='Y' and
								op.op_delivstatus = 'N' and ".$sub_que." p.partnerCode = '".$partnerCode."'";
			...
			error_frame_loc_msg("/?pn=mypage.order.list&" . enc('d' , $_PVSC) , "발송완료 처리하였습니다.");
			break;
		// - 구매완료 ---
		...
		case "coupon_sms_resend":
			...
			// 주문상품과 쿠폰정보
			$osr = _MQ("select op_pname, op_option1, op_option2, op_option3, opc_expressnum, ordernum , ordernum , userhtel1, userhtel2 , userhtel3,  orderhtel1, orderhtel2, orderhtel3, opc_send
						...
						where opc_uid = '".$opcuid."'"); // #LDD020 와일드카드 측에 opc_send 추가

			# LDD020 // 쿠폰 문자 재발송 카운트 기록 및 재발송 제한
			if($row_setup['smsMaxCount'] <= $osr['opc_send']) error_alt("문자발송 횟수를 초과하였습니다.\\n관리자에게 문의 바랍니다.");
			_MQ_noreturn(" update `odtOrderProductCoupon` set `opc_send` = `opc_send` +1 where `opc_uid` = '{$opcuid}' ");
