<?
	...
	switch($_mode) {
		case "join" :
            ...
            if($row_setup['s_join_auth_use'] == 'N') {
                $_ordr_idxx = nullchk($_ordr_idxx , "본인 인증후 회원가입이 가능합니다." , "" , "ALT");
                $res_auth = _MQ(" select * from odtIndividualAuthLog where inl_ordr_idxx = '". $_ordr_idxx ."' ");
                if($res_auth['inl_enc_cert_data']){
                    ...
                    if($_htel <> $phone_no) error_alt('본인 인증 정보가 변조되었습니다.');
                    if($_name <> $user_name) error_alt('본인 인증 정보가 변조되었습니다.');
                    if($_birth <> $birth_day) error_alt('본인 인증 정보가 변조되었습니다.');
                    if($_sex <> $sex_code) error_alt('본인 인증 정보가 변조되었습니다.');
			...
			# 비밀번호 유효성 체크
			$pw_replace = preg_replace("/[a-z0-9@!#\$\?\+-_]/i",'',$_passwd);
			if( strlen(trim($pw_replace))>0 ) { error_alt("비밀번호는 알파벳과 숫자 및 특수문자(@,!,#,$,+,-,_)만 입력 가능합니다."); }

			# 생년월일
			$birth = date('Y-m-d',strtotime($_POST['_birth']));
			$birth	= explode("-", $birth);
			$birthy	= $birth[0];
			$birthm	= $birth[1];
			$birthd	= $birth[2];
			...
			#연락처 정리
			$_htel = rm_str($_htel); $_tel = rm_str($_tel);
			$_htelm = tel_format($_htel); $_htelm = explode('-',$_htelm); $_htel1 = $_htelm[0]; $_htel2 = $_htelm[1]; $_htel3 = $_htelm[2];
			$_telm = tel_format($_tel); $_telm = explode('-',$_telm); $_tel1 = $_telm[0]; $_tel2 = $_telm[1]; $_tel3 = $_telm[2];


			## 아이디 중복 체크
			$rows = _MQ("SELECT count(id) as cnt FROM odtMember WHERE id='$id' AND secession<>'Y'");
			if($rows['cnt']) { error_alt('입력하신 아이디는 이미 사용중입니다.\\n\\n다시 입력해 주세요.'); }
			...
			## 데이타 입력  cpasswd, cpasswd_ck 추가 lcy
			$Query = "
				INSERT INTO odtMember set
					id					= '".$id."',
					hID					= '".$hID."',
			...
			if($result) {
                ...
                if( mailCheck($email) ){
                    ...
                    include_once(dirname(__FILE__)."/mail.contents.join.php"); // 메일 내용 불러오기 ($mailing_content)
			...
			} else { error_alt("회원가입이 정상적으로 진행되지 않았습니다.\\n\\n다시한번 시도해 주시기 바랍니다."); }
		...
		case "modify" :
            ...
            // 2018-10-04 SSJ :: 본인인증 사용 시
            if(
                $row_setup['s_join_auth_use'] == 'Y'
                &&
                (
                    phone_print($row_member["htel1"],$row_member["htel2"],$row_member["htel3"]) <> $_htel
                    ||
                    $row_member['name'] <> $_name
                    ||
                    ($row_member['birthy']."-".$row_member['birthm']."-".$row_member['birthd']) <> $_birth
                    ||
                    $row_member['sex'] <> $_sex
                )
            ) {
			...
			// 비밀번호가 있을때만 처리. cpasswd, cpasswd_ck 추가
			if($_POST['_passwd']) {
				# 비밀번호 유효성 체크
				$pw_replace = preg_replace("/[a-z0-9@!#\$\?\+-_]/i",'',$_POST['_passwd']);
				if( strlen(trim($pw_replace))>0 ) { error_alt("비밀번호는 알파벳과 숫자 및 특수문자(@,!,#,$,+,-,_)만 입력 가능합니다."); }
			...
			if($result) { error_frame_loc_msg("/?pn=mypage.modify.form","회원정보가 수정되었습니다."); }
			else { error_alt("정보수정이 정상적으로 진행되지 않았습니다.\\n\\n다시한번 시도해 주시기 바랍니다."); }
		...
		case "delete":
			// 비밀번호 체크
			if($leave_pw=='') { error_frame_reload("비밀번호를 입력하세요."); }
			...
			case 'cpw' :
				...
				error_frame_loc_msg("/", "정상적으로 비밀번호가 변경되었습니다.");
