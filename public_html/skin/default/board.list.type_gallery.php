<div class="cm_board_gallery">
<? if( count($res)==0 ) { ?>
	<!-- 내용없을경우 모두공통 -->
	<div class="cm_no_conts"><div class="no_icon"></div><div class="gtxt">등록된 내용이 없습니다.</div></div>
	<!-- // 내용없을경우 모두공통 -->
<? } else { ?>
	<ul class="col_num3"><!-- 4개~5개 있음 이미지 세로값 조절해줄것 비율맞춰서 사진이 딱 들어맞도록, 관리자에서 설정가능하도록 -->
	<?
	...
	?>
		...
		<li>
			<div class="post">
				...
				<div class="title"><?=cutstr($app_title,30)?></div>
				<div class="date"><?=date('Y.m.d',strtotime($v['b_rdate']))?></div>
				<div class="click">조회 <strong><?=number_format($v['b_hit'])?></strong></div>
			</div>
		</li>
		<? } ?>
	<? } ?>
