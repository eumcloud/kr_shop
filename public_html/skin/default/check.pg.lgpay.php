<?php
	...
	*/
	...
	$card_fail_order = _MQ_assoc(" select ordernum, paymethod from odtOrder where paymethod in ('C', 'L') and paystatus='N' and canceled='N' and orderdate >= '". date("Y-m-d", strtotime("-1 days"))." 00:00:00' ");

	foreach( $card_fail_order as $k => $v){

	$LGD_OID = $v['ordernum']; //주문번호
    ...
    if ($xpay->TX()) {
		...
		if( "0000" == $xpay->Response_Code() ) { // SSJ : 결제 성공 여부 체크 추가 : 2020-10-28
			// - 결제 성공 기록정보 저장 ---
