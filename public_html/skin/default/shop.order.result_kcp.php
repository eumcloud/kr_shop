...
?>
        ...
        */
        ...
        function jsf__pay_backup(form) // 사용하지 않음
        {
            var RetVal = false;
            if(document.Payplus.object == null)
            {
//                openwin = window.open( "/kcp/files/chk_plugin.html", "chk_plugin", "width=600, height=420, top=300, left=300" );
        ...
        function create_goodInfo()
        {
            ...
            var good_info = "seq=1" + chr31 + "ordr_numb=<?=$ordernum?>" + chr31 + "good_name=<?=$app_product_name?>" + chr31 + "good_cntx=1" + chr31 + "good_amtx=<?= $r[tPrice] ?>";
						document.order_info.bask_cntx.value = "1";
						document.order_info.good_info.value = good_info;
        }
    </script>

<form name="order_info" method="post" action="/pages/shop.order.result_kcp.pro.php" >

<?

if      ($r[paymethod] == "C") { $pay_method = "100000000000"; }   // 신용카드
else if ($r[paymethod] == "L") { $pay_method = "010000000000"; }   // 실시간 계좌이체
//else if ($r[paymethod] == "V") { $pay_method = "0010000000000"; }   // 가상계좌
else if ($r[paymethod] == "V") { $pay_method = "001000000000"; }   // 가상계
...
<input type="hidden" name="good_mny"        value="<?= $r[tPrice] ?>" />           <!-- 결제금액(good_mny) - ※ 필수 : 값 설정시 ,(콤마)를 제외한 숫자만 입력하여 주십시오. -->
<input type="hidden" name="buyr_name"       value="<?= $r[ordername] ?>"/>             <!-- 주문자명(buyr_name) -->
<input type="hidden" name="buyr_mail"       value="<?= $r[orderemail] ?>" />   <!-- 주문자 E-mail(buyr_mail) -->
<input type="hidden" name="buyr_tel1"       value="<?=phone_print($r[ordertel1],$r[ordertel2],$r[ordertel3])?>"/><!-- 주문자 연락처1(buyr_tel1) -->
<input type="hidden" name="buyr_tel2"       value="<?=phone_print($r[orderhtel1],$r[orderhtel2],$r[orderhtel3])?>"/><!-- 휴대폰번호(buyr_tel2) -->
...
//{
?>
    <input type="hidden" name="rcvr_name" value="<?=$r[recname]?>"/>   <!-- 수취인 -->
    <input type="hidden" name="rcvr_tel1" value="<?=phone_print($r[rectel1],$r[rectel2],$r[rectel3])?>"/> <!-- 수취인 연락처 -->
    <input type="hidden" name="rcvr_tel2" value="<?=phone_print($r[rechtel1],$r[rechtel2],$r[rechtel3])?>"/>    <!-- 수취인 핸드폰 -->
    <input type="hidden" name="rcvr_mail" value="<?=$r[recemail]?>" />  <!-- 수취인 이메일 -->
    <input type="hidden" name="rcvr_zipx" value="<?=$r[reczip1]."-".$r[reczip2]?>"/>   <!-- 우편번호 -->
    <input type="hidden" name="rcvr_add1" value="<?=$r[recaddress]?>"/>
    <input type="hidden" name="rcvr_add2" value="<?=$r[recaddress1]?>"/>
    ...
    <?
        $good_info_result = '';
        foreach($good_info as $gk => $gv) {
            $good_info_result .= "seq=";
            $good_info_result .= $gk+1;
            foreach($gv as $gkk => $gvv) { $good_info_result .= ''.$gkk.'='.$gvv; }
            $good_info_result .= '';
        }
    ?>
...
<input type="hidden" name="vcnt_expire_term" value="<?=$row_setup[P_V_DATE]?>"/>
