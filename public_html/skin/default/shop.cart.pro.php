	...
	switch($mode){
		...
		case "promotion_code":
			...
			if($promotion_code=='') { $__result_text = "프로모션코드를 입력하세요."; $__result_code = "FAIL"; promo_output($__result_text,$__result_code); }
				...
				// 존재하는 코드 체크
				$chk = _MQ_result(" select count(*) from odtPromotionCode where pr_code = '".$promotion_code."' and pr_use = 'Y' ");
				if($chk==0) { $__result_text = "잘못된 코드입니다."; $__result_code = "FAIL"; promo_output($__result_text,$__result_code); }

				// 만료여부 체크
				$chk = _MQ_result(" select count(*) from odtPromotionCode where pr_code = '".$promotion_code."' and pr_expire_date >= CURDATE() and pr_use = 'Y' ");
				if($chk==0) { $__result_text = "만료된 코드입니다."; $__result_code = "FAIL"; promo_output($__result_text,$__result_code); }

				// 정보 반환
				$p = _MQ(" select * from odtPromotionCode where pr_code = '".$promotion_code."' and pr_use = 'Y' ");
		...
		case "select_onlydelete":
			$que = "delete from odtCart where c_cookie = '".$_COOKIE["AuthShopCOOKIEID"]."' and c_pcode = '".$code."' ";
			_MQ_noreturn($que);
			error_loc("/?pn=shop.cart.list");
			break;
		...
		case "select_modify":
			...
			if( $app_cnt <= 0 ) { error_msg("수정하실 수량은 0보다 커야 합니다."); }
		...
		// 다수선택삭제
		case "select_delete":
			if( sizeof($_code) == 0 ) { error_msg("1개이상 선택해주시기 바랍니다."); }
		...
		case "select_add":
			...
			if( sizeof($pcode_array) == 0 ) { error_msg("1개이상 선택해주시기 바랍니다."); }
			for($i=0;$i<count($pcode_array);$i++) {
				$pcode = $pcode_array[$i];
				if( !$pass_type ) { $pass_type = "cart"; }
				...
				if($row_product['ipDistinct']) {
					...
					");
					if($orderCheckTmp > 0) { error_msgall("중복구입이 불가능한 상품이 포함되어 있습니다.","/"); exit; }
					else{
			...
			if( $pass_type == "order" ) { error_loc("/?pn=shop.order.form"); }
			else { error_loc("/?pn=shop.cart.list"); }
			break;
		...
		case "add":
			...
			if( $option_select_type == "nooption" ) {
				...
				if($row_product['buy_limit']>0 && $c_cnt > $row_product['buy_limit']) {
					error_msg("선택하신 상품은 최대 " . number_format($row_product['buy_limit']) . "개까지 구매가 가능합니다.");
				}
