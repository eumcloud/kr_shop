<?PHP
	...
	switch($_mode){
		...
		case "login":
			...
			// 아이디 하루동안 저장
			//samesiteCookie("AuthSDIndividualIDChk", "" , time() +3600*24 , "/" , "." . str_replace("www." , "" , $_SERVER['HTTP_HOST']));

			// 아이디저장
			if( $login_id_chk == "Y" ) {
				samesiteCookie("AuthSDIndividualIDChk", $_id , time()+3600*24*30 , "/" , "." . str_replace("www." , "" , $_SERVER['HTTP_HOST']));
			}
			else {
			...
			if($cpwd_day <= date("Y-m-d",time())){ $_move_path=enc('e', 'pn=member.password.form'); } // 체크 변경 끝

			error_frame_loc("/?".enc("d",$_move_path));
		...
		case "logout":
			...
			// --- JJC003 - 묶음배송 :: 장바구니 기록남기기 ---
			samesiteCookie("AuthShopCOOKIEID", substr(enc('e' , md5(time().rand(0,9999)."hy shop")),0,15), 0 , "/" , '.'.str_replace("www." , "" , $_SERVER['HTTP_HOST'])  );//장바구니 원복
			// --- JJC003 - 묶음배송 :: 장바구니 기록남기기 ---
		...
		case "find_idpw" :
			...
			if( mailCheck($email) ){
				...
				include_once(dirname(__FILE__)."/mail.contents.find_idpw.php"); // 메일 내용 불러오기 ($mailing_content)
		...
		// PC 상단 큰 배너 닫기
		case "topbig_banner_close":
				samesiteCookie("AuthPopupClose_topbig" , "Y" ,  time() +3600 * 24, "/" , "." . str_replace("www." , "" , $_SERVER['HTTP_HOST']));
				echo 'OK';
			break;
