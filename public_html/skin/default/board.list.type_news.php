...
<? } else { ?>
	...
	<?
		...
		foreach($res as $k=>$v) {
			...
			foreach($v as $sk=>$sv) {if($sk <> "b_content"){$v[$sk] = htmlspecialchars($sv);}}
	...
	?>
		...
		<? } else { ?>
		<li>
			<a href="#none" onclick="<?=$onclick_event?>" class="upper_link" title="<?=$app_title?>"><img src="/pages/images/cm_images/blank.gif" alt="" /></a>
			<!-- 240x140 -->
			<span class="thumb">
			<? if( $v['b_thumb'] && @filesize(dirname(__FILE__)."/../../upfiles/bbs/".$v['b_thumb']) > 0 ) { ?>
			<img src="<?=IMG_DIR_BOARD.$v['b_thumb']?>" alt=""/>
			<? } ?>
			</span>

			<div class="posting">
				<!-- 1줄제한 -->
				<div class="title"><?=cutstr($app_title,65)?></div>
				<!-- 2줄제한 -->
				<div class="conts"><?=cutstr(stripslashes(strip_tags(str_replace('&nbsp;', ' ', htmlspecialchars_decode($v['b_content'])))),200)?></div>
				<div class="date"><?=date('Y.m.d',strtotime($v['b_rdate']))?></div>
			</div>
		</li>
		<? } ?>
	<? } ?>
