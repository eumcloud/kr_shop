...
<form name=frmAGS_pay method=post action='/pages/shop.order.result_allthegate.pro.php'>
...
<?
if ($r[paymethod] == "C") { $job = "onlycard";	}
else if($r[paymethod] == "L")  { $job = "onlyiche";	}
else if($r[paymethod] == "V")  { $job = "onlyvirtualselfescrow"; }
?>


<input type=hidden name=paymethod			value="<?=$r[paymethod]?>">	<!-- 결제방법 -->
...
<input type=hidden name=Amt					value="<?=$r[tPrice]?>">			<!-- 금액 -->
<input type=hidden name=StoreNm				value="<?=$row_company[name]?>"><!-- 상점명 -->
...
<input type=hidden name=UserId				value="<?=$r[orderid]?>">		<!-- 아이디 -->
<input type=hidden name=UserEmail			value="<?=$r[orderemail]?>">	<!-- 주문자 메일 -->
<input type=hidden name=OrdNm				value="<?=$r[ordername]?>">	<!-- 주문자 -->
<input type=hidden name=OrdPhone			value="<?=phone_print($r[orderhtel1],$r[orderhtel2],$r[orderhtel3])?>">		<!-- 주문자 연락처 -->
<input type=hidden name=RcpNm				value="<?=$r[recname]?>">	<!-- 수취인 -->
<input type=hidden name=RcpPhone			value="<?=phone_print($r[rechtel1],$r[rechtel2],$r[rechtel3])?>">	<!-- 수취인 연락처 -->
<input type=hidden name=DlvAddr				value="<?=$r[recaddress]." ".$r[recaddress1]?>">	<!-- 배송지주소 -->
<input type=hidden name=Remark				value="<?=$r[comment]?>">	<!-- 요구사항 -->
<input type="hidden" name="VIRTUAL_DEPODT" value="<?=date('Ymd', time() + ($row_setup[P_V_DATE] * 86400))?>">
<input type="hidden" name="MallPage" value="/pages/shop.order.result_allthegate_VirAcctResult.php">
