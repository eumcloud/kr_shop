...
<div class="layout_fix">
			...
			<?
				...
				foreach($cr as $k=>$v) {
					...
					foreach($sr as $sk => $sv) {
						...
						if($sv[op_delivstatus] == "N") {
							...
							if($sv['op_orderproduct_type']=='coupon'){
								...
								$delivery_btn_print .= "
								<span class='button_pack'><a href='".$arr_delivery_company[$sv['op_expressname']].rm_str($sv['op_expressnum'])."' target='_blank' class='btn_sm_white'>배송조회</a></span>
								";
							}
						}
						/* 배송상태 처리 끝 */
					...
					if($r['paystatus']=='Y') {
						switch($sv['op_cancel']) {
							case 'Y': // 취소완료
							$app_btn_cancel = "<span class='button_pack'><a href='#none' onclick=\"return false;\" data-ordernum='".$r['ordernum']."' data-opuid='".$sv['op_uid']."' class='btn_sm_white product_cancel_view'>취소내역</a></span>";
							...
							default:
							if($r['canceled']=='N') {
								$app_btn_cancel = "<span class='button_pack'><a href='#none' onclick=\"return false;\" data-ordernum='".$r['ordernum']."' data-opuid='".$sv['op_uid']."' class='btn_sm_white product_cancel'>주문취소</a></span>";
								$app_status_cancel = "";
							}
							break;
						}
						...
						$status_print = $app_status_cancel ? $app_status_cancel : $status_print;
						$delivery_btn_print = $delivery_btn_print . $app_btn_cancel;
					}
			?>
			<tr>
				<td>
					<!-- 상품사진 -->
					<a href="<?=rewrite_url($v['code'])?>" target="_blank" title="<?=$pro_name?>" class="thumb"><? if(app_thumbnail("장바구니",$v)) { ?><img src="<?=$img_src?>" alt="<?=$pro_nam?>" title="<?=$pro_name?>"/><? } ?></a>
					<!-- 상품정보 -->
					<div class="item_name">
						<dl>
	...
	<? if($r['o_promotion_price'] > 0) { //쿠폰, 포인트 할인금액 2015-11-13 LCY002 ?>
	       		<li class="ess">
						...
						<dl>
				                 		<dd>프로모션코드 : <strong><?=number_format($r['o_promotion_price'])?></strong>원<span class="shape"></span></dd>
		...
		<div class="this_item">
			...
			<div class="info">
				...
				<div class="info_price">
					...
					<? // 2016-11-30 ::: 부분취소 - 할인비용 항목 추가 ::: JJC ?>				</div>
			</div>
		</div>
		<!-- / 상품정보 -->
		...
		$.ajax({
			...
			success: function(data) {
				if(data['result']=='OK'){
					...
					if(data['data']['option']) { $product_pop.find('.product_option').text('옵션: ' + data['data']['option']); }
