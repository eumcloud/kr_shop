...
// 거래번호
$TID = ($r['oc_tid'] ? $r['oc_tid'] : $ordr['oc_tid']); // PG사 거래 번호

/* [결제취소 요청 사전 정리] *************/
$CST_PLATFORM			= $row_setup['P_MODE'];       //PG사 결제 서비스 선택(test:테스트, service:서비스)
$MID								= $row_setup['P_ID'];            //상점아이디(PG사 부터 발급받으신 상점아이디를 입력하세요)
/* [결제취소 요청 사전 정리] *************/
...
if($_paymethod=='V') {
	...
	// 마지막 주문취소인지 체크 
	$total_chk = _MQ(" select count(*) as cnt from odtOrderProduct where op_cancel!='Y' and op_oordernum = '".$ordernum."' ");
