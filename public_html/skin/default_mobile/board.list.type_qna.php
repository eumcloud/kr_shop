<div class="cm_board_normal cm_board_qna">
...
<? } else { ?>
	<ul>
		...
		?>
			...
			<? } else { ?>
			<li>
				<a href="#none" onclick="<?=$onclick_event?>" class="upper_link" title="<?=$app_title?>"></a>
				<!-- 새글 아이콘은 기본 48시간 노출 -->
				<? if($is_new) { ?><span class="icon_new">N</span><? } ?>
				<!-- 비밀글일때만 나타남 -->
				<? if($is_secret) { ?><span class="icon_front"><img src="/m/images/cm_images/board_ic_secret.png" alt="비밀글" /></span><? } ?>
				<!-- 질문과답변일때 -->
				<span class="icon_reply"><span class="texticon_pack"><span class="<?=$icon?>"><?=$status?></span></span></span>
				<div class="title"><?=$_title?></div>
				<span class="writer"><span class="name"><?=$v['b_writer']?></span><span class="bar"></span><span class="date"><?=date('Y-m-d',strtotime($v['b_rdate']))?></span></span>					
			</li>
			<? } ?>
		<? } ?>
	</ul>
