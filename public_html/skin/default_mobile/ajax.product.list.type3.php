...
<?
	...
	echo "<div class='list_thumb_l'><ul>";
	// - 넘길 변수 설정하기 ---
	$_PVS = ""; // 링크 넘김 변수
	...
	foreach($assoc as $key => $row) {
		...
		$image				= replace_image(IMG_DIR_PRODUCT.$row['prolist_img2']); // 상품이미지
...
<li>
	<div class="item_box">
		...
		<div class="thumb">
			...
			<? }} ?>
			<div class="thumb_img" style="<?=$row['v_color']?'background-color:#'.$row['v_color']:''?>">
				<? if($row['prolist_img']){ ?><img class="lazy" data-original="<?=$image?>" alt="<?=$row['name']?>"/><? } ?>
			</div>
		</div>
		...
		<div class="info">
			...
			<div class="price">
				...
				<span class="before"><del><?=number_format($price_org)?></del></span>
				<? } ?>
				<span class="after"><strong><?=number_format($price)?></strong><!-- 원 --></span>
			</div>
		</div>
	</div>
</li>
