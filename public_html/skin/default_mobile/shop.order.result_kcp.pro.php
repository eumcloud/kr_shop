<?PHP
	...
	// -- 주문번호 패치 2017-08-03 LCY --
	$ordernum = $ordernum != '' ? $ordernum :  trim($_POST[ "ordr_idxx"      ]);

	// 주문정보 추출
	$r = _MQ("select * from odtOrder where ordernum='". $ordernum ."' ");

	// 결제금액이 정상인지 체크
	if($_POST[good_mny] != $r[tPrice]) {
		error_loc_msg("/?pn=shop.order.result" , "결제금액이 다릅니다. 정상결제금액 : ".$r[tPrice].", 요청된결제금액 : ".$_POST[good_mny]);
	}
  ...
  /* = -------------------------------------------------------------------------- = */
	...
	if ( $req_tx == "pay" )
	{
			if( $res_cd == "0000" )
			{
					...
					if ( $use_pay_method == "001000000000" )
					{
							...
							$app_time  = $c_PayPlus->mf_get_res_data( "app_time"  ); // 승인 시간
							$bank_code = preg_replace( '/[^0-9]/', '', $bankcode );
					}
	...
	if ( $req_tx == "pay" )
	{
			if( $res_cd == "0000" )
			{
				_MQ_noreturn("update odtOrder set taxorder='".$cash_yn."' where ordernum='".$ordernum."'");
				if($cash_yn=='Y') {

					if($cash_authno) {
	            	    _MQ_noreturn("insert into odtOrderCashlog (ocs_ordernum,ocs_member,ocs_date,ocs_cashnum,ocs_tid,ocs_amount,ocs_method,ocs_type) values ('$ordernum','$r[orderid]',now(),'$cash_authno','$amount','AUTH','iche')");
	        		}
		        }
...
// 주문완료시 처리 부분 - 주문서수정,포인트,수량,문자발송,메일발송
						...
						_MQ_noreturn("update odtOrder set authum = '$app_no' where ordernum = '$ordernum'");
				}
	...
	//$oc_res_cnt = _MQ(" select count(*) as cnt from odtOrderCardlog where oc_oordernum = '".$ordernum."' and oc_tid = '". $resultMap['tid']."' and oc_content like '%결과코드||0000%' ");
	$oc_res_cnt = _MQ(" select count(*) as cnt from odtOrderCardlog where oc_oordernum = '".$ordernum."' and oc_tid != '' and oc_content like '%결과코드||0000%' ");
	if($oc_res_cnt['cnt'] == 1 ) {
		...
		error_loc_msg("/?pn=shop.order.result" , "결제에 실패하였습니다. 다시 한번 확인 바랍니다.");
