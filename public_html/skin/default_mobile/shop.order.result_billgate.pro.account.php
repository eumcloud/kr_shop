...
if(!strcmp($RESPONSE_CODE, "0000")) { // If authentication is successful, the payment request
	...
	$AUTH_AMOUNT 		= "";
	$AUTH_DATE 			= "";
	unset($AUTH_DATE,$AUTH_AMOUNT);
	
	$USING_TYPE 		= "";
	$IDENTIFIER_TYPE= "";
	$AUTH_DATEIDENTIFIER 		= "";
	unset($USING_TYPE,$IDENTIFIER_TYPE,$AUTH_DATEIDENTIFIER);
...
echo "<script language='javascript'>location.href=('/m/?pn=shop.order.complete');</script>";
...
//---------------------------------------
	...
	_MQ_noreturn("update smart_order set orderstatus_step='결제실패' where ordernum='". $ordernum ."' ");
	echo "<script language='javascript'>alert('결제에 실패하였습니다. 다시 한번 확인 바랍니다.');location.href=('/m/?pn=shop.order.result');</script>";
