<?PHP
	...
	// 사후 체킹1 - 결제금액 <> (구매총액 + 배송비 - 할인총액)
	if( $price_total <> ($price_sum + $price_delivery - ($_use_point + $use_coupon_price_member + $use_coupon_price_product)) ) {
		error_loc_msg("/m/?pn=shop.order.form","결제금액이 맞지 않습니다.");
	}
	// 사후체킹1-2 : 결제금액이 0이고 할인금액이 0보다클때만 전액적립금결제 허용
	if($_paymethod == "point"){
		if( !($price_total == 0 && ($_use_point + $use_coupon_price_member + $use_coupon_price_product) > 0) ){
			error_loc_msg("/m/?pn=shop.order.form","전액 적립금 결제는 선택할수 없습니다. ");
		}
	}
	...
	if( $_SESSION["order_start"] == $_COOKIE["AuthShopCOOKIEID"]){
		...
		// 포인트 사용량 체크
		if( $_price_usepoint > $row_member['point'] ) { error_loc_msg("/m/?pn=shop.order.form","소유한 포인트보다 사용포인트량이 많습니다."); }

		// 품절 체크
		$is_soldout= _MQ_result("select min(p.stock) from odtCart as c
									left join odtProduct as p on (p.code = c.c_pcode)
									where c_cookie='".$_COOKIE["AuthShopCOOKIEID"]."' and c_direct='Y' ");// 선택 구매 2015-12-04 LDD
		if($is_soldout < 1) { error_loc_msg('/m/','해당 상품은 품절되었습니다.'); }
		...
		$sres = _MQ_assoc(" select c.*,p.name,p.setup_delivery,p.customerCode , p.comSaleType, p.commission from odtCart as c
							left join odtProduct as p on (p.code = c.c_pcode)
							where c_cookie='".$_COOKIE["AuthShopCOOKIEID"]."'  and c_direct='Y' ");// 선택 구매 2015-12-04 LDD
		foreach( $sres as $k=>$v ){
		...
		include_once(dirname(__FILE__)."/../../pages/shop.order.usepoint.php");
		...
		switch($_paymethod) {
			case "online" :
				...
				if($row_sms[$smskbn]['smschk'] == "y") {
					...
					// 치환작업
					$sms_msg	= str_replace("{{주문번호}}",$_ordernum,$sms_msg);
					$sms_msg	= str_replace("{{결제금액}}",number_format($_price_real),$sms_msg);
					$sms_msg	= str_replace("{{입금계좌정보}}",$_paybankname,$sms_msg);
					$sms_msg	= str_replace("{{사이트명}}",$row_setup['site_name'],$sms_msg);
					$sms_msg = str_replace("{{주문상품명}}",$SMSProduct['op_pname'],$sms_msg); // 2016-07-19 LDD
					$sms_msg	= str_replace("{{주문상품수}}",$SMSProductCnt ,$sms_msg);
				...
				if($row_sms[$smskbn]['smschk'] == "y") {
					...
					// 치환작업
					// $sms_msg	= str_replace("{{주문번호}}",$_ordernum,$sms_msg);
					// $sms_msg	= str_replace("{{구매자명}}",$_oname,$sms_msg);
					// $sms_msg	= str_replace("{{결제금액}}",number_format($_price_real),$sms_msg);
					// $sms_msg	= str_replace("{{사이트명}}",$row_setup['site_name'],$sms_msg);
					$sms_msg = sms_msg_replace($sms_msg, $_ordernum); // LDD008
			...
			case "point" :	// 전액 포인트 결제
				...
				// 제공변수 : $_ordernum
				include_once(dirname(__FILE__)."/../../pages/shop.order.pointadd_pro.php");

				// 상품 재고 차감 및 판매량 증가
				$_ordernum = $_ordernum;
				include_once(dirname(__FILE__)."/../../pages/shop.order.salecntadd_pro.php");
				...
				// 제공변수 : $_ordernum
				$_ordernum = $_ordernum;
				include_once(dirname(__FILE__)."/../../pages/shop.order.couponadd_pro.php");

				// 결제완료 문자발송
				$_ordernum = $_ordernum;
				include_once(dirname(__FILE__)."/../../pages/shop.order.sms_send.php");
	...
	if( $_paymethod == "online" || $_paymethod == "point") { error_loc("/m/?pn=shop.order.complete"); }
	else { error_loc("/m/?pn=shop.order.result"); }
?>