...
if($trcode == '1') { // 입금
	...
	if($r[ool_amount_total] == $r[ool_amount_current]) {
		...
		if($row_sms[$smskbn][smschk] == "y") {
			...
			// 치환작업.
			// $sms_msg	= str_replace("{{주문번호}}", $r[ordernum] ,$sms_msg);
			// $sms_msg	= str_replace("{{결제금액}}",number_format($r[tPrice]),$sms_msg);
			// $sms_msg	= str_replace("{{구매자명}}",$r[ordername],$sms_msg);
			// $sms_msg = str_replace("{{사이트명}}",$row_setup[site_name],$sms_msg);
			$sms_msg = sms_msg_replace($sms_msg, $r['ordernum']); // LDD008
		...
		if( mailCheck($_oemail) ){
			...
			//$_title_img = "/pages/images/mailing/title_order.gif";
