<?PHP
	...
	if( in_array( $_mode , array("add" , "modify" , "reply") ) ){
        ...
        //이미지업로드처리
        $img_name1 = _PhotoPro( dirname(__FILE__)."/../../upfiles/board" , "_img1" );
        $img_name2 = _PhotoPro( dirname(__FILE__)."/../../upfiles/board" , "_img2" );
	...
	if( $_uid) {
		...
		if(!$is_auth && $_mode <> "reply" ) {
			...
			if($r[b_passwd] != $_passwd) 	error_alt("비밀번호가 일치하지 않습니다.");
		}
	...
	switch( $_mode ){
		...
		// 등록
		case "add":
			$que = " insert odtBbs set {$sque} ,b_inid = '" . get_userid() . "', b_writer_type = '". (is_login() ? "member" : "guest") ."' , b_passwd = password('". $_passwd ."') , b_rdate = now() ";
			_MQ_noreturn($que);
		...
		case "delete":
            ...
            //이미지파일삭제
            _PhotoDel( dirname(__FILE__)."/../../upfiles/board" , $r[b_img1]);
            _PhotoDel( dirname(__FILE__)."/../../upfiles/board" , $r[b_img2]);
        ...
        // 댓글
		case "reply":
			$que = " insert odtBbs set {$sque} , b_inid = '" . get_userid() . "' , b_depth='2' , b_relation='".$r[b_uid]."'  ,b_rdate = now() , b_passwd = password('". $_passwd ."') ";
			_MQ_noreturn($que);
