...
<div class="common_inner common_full">
	...
	<div class="cm_mypage_ctrl">		
		<span class="button_pack"><a href="#none" onclick="return false;" class="select_none btn_sm_white">선택해제</a></span>
		<span class="button_pack"><a href="#none" onclick="return false;" class="select_all btn_sm_white">전체선택</a></span>
		<span class="button_pack"><a href="#none" onclick="return false;" class="select_delete btn_sm_black">선택삭제</a></span>
	</div>
	<!-- // 리스트 제어버튼영역 -->
	...
	<ul>
		...
		<li id="wish_<?=$v['code']?>">
			<div class="wish_box">
				<label><input type="checkbox" name="_wish" value="<?=$v['code']?>"/></label>
				<dl>
					<dt>
						<a href="<?=rewrite_url($v['code'])?>" class="upper_link" target="_blank"></a>
						<div class="thumb"><? if($v['prolist_img']){ ?><img src="<?=replace_image(IMG_DIR_PRODUCT.$v['prolist_img'])?>" alt="<?=$v['name']?>" title="" /><? } ?></div>
						<div class="title"><?=$v['name']?></div>
					</dt>
					<dd>
						<span class="price"><?=number_format($v['price'])?>원</span>
						<span class="button_pack"><a href="#none" onclick="return false;" data-code="<?=$v['code']?>" class="ajax_wish_del btn_sm_white">찜삭제</a></span>
					</dd>
				</dl>
			</div>
		</li>
