...
<div class="common_inner common_full">
	...
	<div class="cm_mypage_sumbox">
		<dl>
			<dt>총 절약금액 <?=number_format($save_price)?>원</dt>
			<dd>절약금액(<?=number_format($save_price)?>원)<br/>= 정상가격(<?=number_format($order_info[sum_org])?>원) - 할인구매가격(<?=number_format($order_info[sum_price])?>원)</dd>
		</dl>
	</div>
	<!-- / 페이지 통계박스 -->
	...
	<div class="cm_order_search">
		...
		<div class="period">
			<ul>
				<li class="<?=$_period=='all'?'hit':''?>"><a href="/m/?pn=<?=$pn?>&_mode=<?=$_mode?>&_period=all" class="btn">전체</a></li>
				<li class="<?=$_period=='today'?'hit':''?>"><a href="/m/?pn=<?=$pn?>&_mode=<?=$_mode?>&_period=today" class="btn">오늘</a></li>
				<li class="<?=$_period=='week'?'hit':''?>"><a href="/m/?pn=<?=$pn?>&_mode=<?=$_mode?>&_period=week" class="btn">1주일</a></li>
				<li class="<?=$_period=='month'?'hit':''?>"><a href="/m/?pn=<?=$pn?>&_mode=<?=$_mode?>&_period=month" class="btn">1개월</a></li>
				<li class="<?=$_period=='three'?'hit':''?>"><a href="/m/?pn=<?=$pn?>&_mode=<?=$_mode?>&_period=three" class="btn">3개월</a></li>
			</ul>
		</div>
		...
		<div class="detail">
			...
			<div class="input_wrap">
				<ul>
					<li><input type="text" name="_period_s" id="s_date" readonly value="<?=$_period_s?>" class="input_date"/></li>
					<li><input type="text" name="_period_e" id="e_date" readonly value="<?=$_period_e?>" class="input_date"/></li>
				</ul>					
			</div>
			<span class="button_pack"><a href="#none" onclick="return false;" class="submit_period btn_md_black">조회</a></span>
			</form>
		</div>
		<script>
	...
	<div class="cm_mypage_tab">
		<div class="inner_tab">
			<ul>
				<li class="<?=$_mode=='all'?'hit':''?>"><a href="/m/?pn=<?=$pn?>&_mode=all&_period=<?=$_period?>&_period_s=<?=$_period_s?>&_period_e=<?=$_period_e?>">전체주문<strong><?=number_format($all_cnt)?></strong></a></li>
				<li class="<?=!$_mode?'hit':''?>"><a href="/m/?pn=<?=$pn?>">진행중인주문<strong><?=number_format($on_cnt)?></strong></a></li>
				<li class="<?=$_mode=='done'?'hit':''?>"><a href="/m/?pn=<?=$pn?>&_mode=done&_period=<?=$_period?>&_period_s=<?=$_period_s?>&_period_e=<?=$_period_e?>">완료된주문<strong><?=number_format($done_cnt)?></strong></a></li>
				<li class="<?=$_mode=='cancel'?'hit':''?>"><a href="/m/?pn=<?=$pn?>&_mode=cancel&_period=<?=$_period?>&_period_s=<?=$_period_s?>&_period_e=<?=$_period_e?>">취소된주문<strong><?=number_format($cancel_cnt)?></strong></a></li>
			</ul>
		</div>
	</div>
	<!-- //카테고리있을경우 게시판메뉴 -->
	...
	<div class="cm_order_list">
		...
		<dl>
			<dd>
				<a href="/m/?pn=mypage.order.view&ordernum=<?=$v['ordernum']?>&_PVSC=<?=$_PVSC?>" class="upper_link"></a>
				<!-- 상품정보 -->
			...
			<dt>
				<span class="btn_box">
					<ul>
						<li><span class="button_pack"><a href="/m/?pn=mypage.order.view&ordernum=<?=$v['ordernum']?>&_PVSC=<?=$_PVSC?>" class="btn_sm_black">주문상세</a></span></li>
						<? if($v['canceled'] != "Y" && $v['paystatus2'] == "N" && !in_array($v['orderstatus_step'] , array('발송완료' , '발급완료')) ) { if($v['mem_cancelchk'] == "Y") { ?>
						<!-- 중복 주석처리 LCY101 <li><span class="button_pack"><a href="#none" onclick="order_cancel('<?=$v['ordernum']?>');return false;" class="btn_sm_white">주문취소</a></span></li>-->
							<?
								$cancel_chk = _MQ_result(" select count(*) from odtOrderProduct where op_oordernum = '".$v['ordernum']."' and op_cancel != 'N' ");
								if( $cancel_chk > 0 ) { 
							?>
							<li><span class="button_pack"><a href="#none" onclick="alert('취소/반품/교환 요청중인 상품이 있습니다. 개별상품 단위로 요청해주시기 바랍니다.');return false;" class="btn_sm_white">주문취소</a></span></li>
							<? } else { ?>
