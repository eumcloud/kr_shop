...
<div class="cm_faq_search">
	...
	<div class="gtxt_box">
		<div class="telnumber">
			고객센터<a href="tel:<?=$row_company['tel']?>" class="call"><?=$row_company['tel']?></a>
		</div>
		<div class="explain">사이트 이용에 관한 궁금한 내용을 먼저 검색해보세요. 원하는 정보가 없거나 더 자세한 내용을 원하는 분들은
			<strong><a href="/m/?pn=mypage.request.form">1:1온라인문의</a></strong>를 이용하거나, <strong>고객센터</strong>로 연락주십시오.
		</div>
	</div>

	<div class="search_form">
		<form name="bbs_search" method="get" action="/m/" role="search">
		<input type="hidden" name="pn" value="board.list"/>
		<input type="hidden" name="_menu" value="<?=$b_menu?>"/>
		<? if($search_word) { ?>
		<!-- 검색한 후 전체로 돌아갈때 -->
		<a href="/m/?pn=<?=$pn?>&_menu=<?=$_menu?>" class="btn_viewall">전체보기</a>
		<? } ?>
		<div class="input_box">
			<input type="search" name="search_word" value="<?=stripslashes($_GET['search_word'])?>" class="input_design" placeholder="궁금한점을 검색해보세요." />
...
<div class="cm_board_tab">
	<ul>
	<li class="<?=!$_category?'hit':''?>"><a href="/?pn=board.list&_menu=faq&_category=" class="upper_link"></a>전체보기</li>
	<? foreach($arr_board_category['faq'] as $k=>$v) { ?>
	<li class="<?=$_category==$v?'hit':''?>"><a href="/?pn=board.list&_menu=faq&_category=<?=$v?>" class="upper_link"></a><?=$v?></li>
	<? } ?>
	</ul>
</div>
<!-- //카테고리있을경우 게시판메뉴 -->
...
?>
	<li class="faq_item" id="faq_<?=$v['b_uid']?>">
		<a href="#none" onclick="faq_view('<?=$v['b_uid']?>');return false;" class="upper_link"></a>
		<span class="state"><span class="state_icon">Q</span></span>
		<span class="question"><!-- <span class="category">[개인회원]</span> --><?=$v['b_title']?></span>
		<div class="answer"><span class="shape_ic"></span><?=$v['b_content']?></div>
	</li>
<? } ?>
	</ul>
<? } ?>
