<?php
//if(substr(phpversion(),0,3) < 5.4) { session_register("order_start"); } // 2016-02-02 LDD
//$_SESSION["order_start"] = $_COOKIE["AuthShopCOOKIEID"]; // 2016-02-02 LDD
						...
						if(trim($pr['coupon_title']) && $pr['coupon_price'] > 0) {
							$product_coupon_html = "
								<div class='item_coupon'>
									<label>
										<input type='checkbox' class='product_coupon_check' onclick='app_order_price();' name='product_coupon[".$pr['code']."]' value='".floor($sum_price*($pr['coupon_price']/100))."'/>
										<span class='white_box'>
											<span class='coupon_ti'>적용가능쿠폰</span>
											<span class='coupon_name'>".stripslashes($pr['coupon_title'])."</span>
										</span>
										<span class='color_box'>
											<span class='edge1'></span><span class='edge2'></span>
											<span class='coupon_discount'>".$pr['coupon_price']."% 할인</span>
											<span class='coupon_price'>".number_format(floor($sum_price*($pr['coupon_price']/100)))."원</span>
										</span>
									</label>
								</div>
							";
...
?>
	...
	<div class="cm_shop_cart_sum">
		...
		<span class="box plus_box">
			...
			<span class="price"><strong><?=number_format($arr_product_sum["delivery"]+$arr_product_sum["add_delivery"])?></strong><em>원</em></span>
		</span>
		...
		<span class="box equal_box">
			...
			<span class="price"><strong id="ID_total_price_smallsum"><?=number_format($arr_product_sum["sum"] + $arr_product_sum["delivery"] + $arr_product_sum["add_delivery"])?></strong><em>원</em></span>
		</span>
	</div>
	<!-- / 계산하기 -->
	...
	<div class="cm_order_agree">
		<div class="text_box"><textarea cols="" rows="" readonly><?=stripslashes($row_company['guideinfo'])?></textarea></div>
		<div class="agree_check">
			<label><input type="checkbox" name="order_guideinfo" id="order_guideinfo" class="" value="Y"/> 위 방침을 읽고 동의합니다.</label>
		</div>
	</div>
	<!-- / 동의하기 -->
	<? // SSJ: 2017-09-20 비회원주문시 이용약관 추가 ?>
	...
	<!-- ●●●●●●●●●● 단락타이틀 -->
	<div class="cm_shop_title">
		<strong>비회원 주문</strong> 개인정보 수집동의
		<div class="explain">비회원으로 주문하실 경우 다음 개인정보 수집방침에 동의해 주셔야 합니다.</div>
	</div>
	<!-- / 단락타이틀 -->
	<!-- ●●●●●●●●●● 비회원일경우 약관동의하기 필요하면 사용 -->
	<div class="cm_order_agree">
		<div class="text_box"><textarea cols="" rows="" readonly><?=stripslashes($row_company['guestinfo'])?></textarea></div>
		<div class="agree_check">
			...
			<ul>
				<li>
					...
					<span class="apply_point">
						<!-- 천단위 콤마 찍기 -->
						<input type="number" pattern="\d*" name="_use_point" class="" value="0" <?=$able_point < $row_setup['paypoint'] ? "disabled" : "";?> />
						<span class="button_pack"><a href="#none" onclick="return false;" class="do_point_apply btn_md_black">할인적용</a></span>
						<span class="button_pack"><a href="#none" onclick="return false;" class="do_point_reset btn_md_white">적용취소</a></span>
					</span>
				</li>
			</ul>
			<script>
	...
	<div class="cm_shop_last_sum">
		...
		<? if( is_login() ) { ?>
		<div class="save_point">본 주문으로 예상되는 적립금: <strong><?=number_format($arr_product_sum["point"])?></strong>원</div>
		<? } ?>
	</div>
	<!-- / 최종계산 -->
	...
	<div class="cm_order_form">
		...
		<ul>
			...
			<li class="ess">
				...
				<div class="value"><input type="text" name=""  pattern="\d*" class="input_design" placeholder="주문자 휴대폰번호" value="<?=phone_print($row_member['htel1'],$row_member['htel2'],$row_member['htel3'])?>" readonly/></div>
			</li>
			<li class="ess">
				<span class="opt">주문자 이메일</span>
				<div class="value"><input type="email" name="" class="input_design" placeholder="주문자 이메일주소 (아이디@주소)" value="<?=$row_member['email']?>" readonly/></div>
			</li>
		</ul>
		<? } else { ?>
		<ul>
			...
			<li class="ess">
				<span class="opt">주문자 휴대폰</span>
				<div class="value"><input type="tel" pattern="\d*" name="_ohtel" class="input_design" placeholder="주문자 휴대폰번호" /></div>
			</li>
			<li class="ess">
				<span class="opt">주문자 이메일</span>
				<div class="value"><input type="email" name="_oemail" class="input_design" placeholder="주문자 이메일주소 (아이디@주소)"/></div>
			</li>
		</ul>
		<? } ?>
	</div>
	...
	<div class="cm_order_form">
		<ul>
			...
			<li class="ess">
				...
				<div class="value"><input type="tel" pattern="\d*" name="_uhtel" class="input_design" placeholder="사용자 휴대폰번호" /></div>
			</li>
			<li class="ess">
				<span class="opt">사용자 이메일</span>
				<div class="value"><input type="email" name="_uemail" class="input_design" placeholder="사용자 이메일주소 (아이디@주소)"/></div>
			</li>
		</ul>
	</div>
	<script>
		...
		<ul>
			...
			<li class="ess">
				...
				<div class="value">
					...
					?>
					<!-- 이전주소 선택할 경우 -->
					<label class="before_address_toggle"><input type="radio" name="_rtype" value="old" id="_rtype_old" />이전사용 주소선택</label>
				</div>
				<div class="before_address" style="display:none;">
					<!-- 이전주소목록 선택하면 나옴, 선택을 해야 닫힘 -->
					<? foreach($sores as $srk=>$srv) { ?>
						<div class="data before_address_apply"
				...
				<script>
				$(document).ready(function(){
					$('.before_address_toggle').on('click',function(e){ e.preventDefault(); $(this).prop('checked',false); $('.before_address').slideDown('fast','easeInOutCubic'); });
					$('#_rtype_equal, #_rtype_new').on('click',function(){ $('#_rtype_old').prop('checked',false); $('.before_address').slideUp('fast','easeInOutCubic'); });
					$('.before_address_apply').on('click',function(){
						$('#_rtype_old').prop('checked',true); $('.before_address').slideUp('fast','easeInOutCubic');
			...
			<li class="">
				...
				<div class="value">
					...
					<input type="text" name="delivery_date" class="input_design" id="delivery_date" disabled readonly>
				</div>
				<script>
					$(function() {
			...
			<li class="ess">
				...
				<div class="value"><input type="tel" pattern="\d*" name="_rhtel" class="input_design" placeholder="받는분 휴대폰번호" /></div>
			</li>
			<li class="ess">
				<span class="opt">받는분 이메일</span>
				<div class="value"><input type="email" name="_remail" class="input_design" placeholder="받는분 이메일주소 (아이디@주소)"/></div>
			</li>
			<li class="ess">
				...
				<div class="value">
					...
					<input type="text" name="_raddress" id="_addr1" readonly class="input_design" placeholder="기본주소"/>
					<input type="text" name="_raddress1" id="_addr2" class="input_design" placeholder="나머지 주소" />
						<div class="tip_txt">
							<dl>
	...
	<div class="cm_order_form">
		<div class="thisis_price">
			<span class="txt">최종결제 금액확인</span>
			<div class="price">
				<span class="lineup">
					<span class="unit_front">￦</span><strong id="ID_total_price2"><?=number_format($arr_product_sum["sum"] + $arr_product_sum["delivery"] + $arr_product_sum["add_delivery"])?></strong><span class="unit">원</span>
				</span>
			</div>
		</div>
		<ul>
			...
			<li>
				<div class="payway">
					<dl>
						<dd class="normal"><label class="use_card"><input type="radio" name="_paymethod" value="card" id="_paymethod_card" checked/>신용카드</label></dd>
<!-- 						<dd class="normal"><label class="use_real"><input type="radio" name="_paymethod" value="iche" id="_paymethod_iche"/>실시간 계좌이체</label></dd> -->
						<dd class="normal"><label class="use_vert"><input type="radio" name="_paymethod" value="virtual" id="_paymethod_virtual"/>가상계좌</label></dd>
						<dd class="normal"><label class="use_bank"><input type="radio" name="_paymethod" value="online" id="_paymethod_online"/>무통장 입금</label></dd>
						<dd class="allpoint" style="display:none;"><label class="use_point"><input type="radio" name="_paymethod" value="point" id="_paymethod_point"/>전액 적립금결제</label></dd>
					</dl>
				</div>
			</li>
			<li class="ess ID_paymethod_online" style="display:none;">
				...
				<div class="value">
					<div class="select">
						...
						echo _InputSelect("_bank",array_values($arr_bank),""," ","","- 계좌 선택 -");
						?>
					</div>
				</div>
			</li>
			<li class="ess ID_paymethod_online" style="display:none;">
				...
				<div class="value">
					<input type="text" name="paydate" id="paydate" class="input_design" value="<?=date('Y-m-d')?>" readonly/>
			...
			<li class="ess ID_paymethod_online" style="display:none;">
				...
				<div class="value">
					<input type="text" name="_deposit" class="input_design" value="<?=$row_member['name']?>" />
	...
	<div class="cm_user_guide">
		<dl>
			...
			<dd>결제방법은 <strong>카드결제, 가상계좌, 무통장입금, 전액적립금결제</strong>가 있습니다.</dd>
	...
	<div class="cm_order_agree">
		<div class="agree_check">
			<label>
				...
				<font color="#ff0000">구매하실 상품의 상품정보 및 가격을 확인하였으며, 이에동의합니다.</font> (전자상거래법 제8조 제2항)
			</label>
		</div>
	</div>
	...
	<div class="cm_bottom_button">
		<ul>
			<li><span class="button_pack"><a href="#none" onclick="if(confirm('작성중인 주문정보가 있습니다.\n이전페이지로 이동하시겠습니까?')){location.href=('/?pn=shop.cart.list');}return false;" title="" class="btn_lg_white">이전단계<span class="edge"></span></a></span></li>
			<li><span class="button_pack"><a href="#none" onclick="order_submit();return false;" title="" class="btn_lg_color">결제하기<span class="edge"></span></a></span></li>
		</ul>
	</div>
	<!-- // 가운데정렬버튼 -->
...
function app_order_all_point(){
	...
	if(($("input[name=price_total]").val()*1 == 0) && ( ($("input[name=_use_point]").val().replace(/,/g,'')*1 + $("input[name=use_coupon_price_member]").val()*1 + $("input[name=use_coupon_price_product]").val()*1 + $("input[name=use_promotion_price]").val().replace(/,/g,'')*1 ) > 0)) {
		...
		//if($("#_paymethod_point").is(':checked')) {
