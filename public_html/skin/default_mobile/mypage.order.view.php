					...
					foreach($sr as $sk => $sv) {
						...
						if( $chk_pcode <> $sv['op_pcode'] ) {
							if($sv['op_orderproduct_type'] != "product") {	// 배송적용 상품이 아니면
								...
								$delivery_print = "<strong>0</strong>원";
								// 추가배송비 여부
								unset($add_delivery_print);
								if($sv['op_add_delivery_price']) {
									$add_delivery_print = "<div class='guide_txt'>추가배송비+".number_format($sv['op_add_delivery_price'])."원</div>";
								}
							}
			...
			<li class="toggle_target" id="toggle_<?=$sv['op_uid']?>">
				...
				<div class="item_charge">
					<dl>
						...
						<? if($v['op_orderproduct_type'] == "product") { ?>
						<dd>
				...
				<li>
					...
					<div class="value">
                        <?php if(in_array($row_setup['P_KBN'],array('I','A','K','L','B','D'))) { ?>
                            ...
                            <?php } ?>
                        <?php } ?>
						<label class="save_check"><input type="radio" name="cancel_type" class="cancel_type_point" checked value="point"/>적립금 환불</label>
					</div>
				</li>
				<?php if( !in_array($r['paymethod'],array('C','L','G')) ) { ?>
