...
//    $uid1 = $_POST[uid1];
    ...
    if( ( str_replace("depth","",$r[option_type_chk]) - $depth ) > 1 ) {
        ...
        echo "<div class='pv_select'>
                <span class='ic_arrow'></span>
				<select name=_option_select" . $depth_next . " ID='option_select" . $depth_next . "_id' class='add_option' onchange=\"option_select(" . $depth_next . ",'".$code."')\"><option value=''>".($arr_option_title[$depth_next] ? $arr_option_title[$depth_next] : $depth_next."차옵션을 선택하세요") ."</option>" . $str_option . "</select>
			</div>";
    ...
    // 재고 추출
        ...
        echo "<div class='pv_select'>
                <span class='ic_arrow'></span>
                    <select name=_option_select" . $depth_next . " ID='option_select" . $depth_next . "_id' class='add_option' onchange=\"option_select_add('".$code."');\" ><option value=''>".($arr_option_title[$depth_next] ? $arr_option_title[$depth_next] : "상세옵션을 선택하세요")."</option>" . $str_option . "</select>
                </span>
            </div>";
    }
