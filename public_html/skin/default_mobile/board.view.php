<?
	...
	// 노출여부 확인
	if($board_info['bi_view'] == "N") error_msg("해당 게시물은 비공개 상태로 볼 수는 권한이 없습니다.\\n궁금하신 점이 있으시다면\\n고객센터(". $row_company['tel'] .")로 문의주시기 바랍니다.");
...
<div class="common_inner common_full">
	...
	<div class="cm_board_view">
		...
		<div class="post_title">
			...
			if(in_array($board_info['bi_list_type'],array('event','event_thumb')) && $r['b_notice']!='Y') {
				if($r['b_sdate'] <= date('Y-m-d') && $r['b_edate'] >= date('Y-m-d')) { $icon = "red"; $status="진행중"; }
				else { $icon = "light"; $status="마감됨"; }
			?>
			<!-- 이벤트일때만 나옴 -->
			<span class="texticon_pack checkicon"><span class="<?=$icon?>"><?=$status?></span></span>
			<? } ?>
			<!-- <div class="txt"><?=stripslashes(htmlspecialchars($r['b_title']))?></div> -->
			<?=stripslashes($r['b_title'])?>
		</div>

		<!-- 글 기본정보 -->
		<div class="post_info">
			<span class="one_tx">작성자<b><?=$r['b_writer']?></b></span>
			<span class="one_tx">작성일<b><?=date('Y.m.d',strtotime($r['b_rdate']))?></b></span>
			<span class="one_tx">조회수<b><?=number_format($r['b_hit']+1)?></b></span>
			<? if(in_array($board_info['bi_list_type'],array('event','event_thumb')) && $r['b_notice']!='Y') { ?>
			<span class="one_tx">이벤트 기간<b><?=date('Y.m.d',strtotime($r['b_sdate']))." ~ ".date('Y.m.d',strtotime($r['b_edate']))?></b></span>
			<? } ?>
		</div>
		...
		<?
			if($r['b_img1_loc'] == "top" && $r['b_img1']) {
				if( @filesize($_dir."/".$r['b_img1']) > 0 ) { echo "<img src='".IMG_DIR_BOARD.$r['b_img1']."' alt=''/>"; }
			}
			if($r['b_img2_loc'] == "top" && $r['b_img2']) {
				if( @filesize($_dir."/".$r['b_img2']) > 0 ) { echo "<img src='".IMG_DIR_BOARD.$r['b_img2']."' alt=''/>"; }
			}

			if( $r['b_thumb'] && @filesize($_dir."/".$r['b_thumb']) > 0 ) {
				echo "<img src='".IMG_DIR_BOARD.$r['b_thumb']."' alt=''/>";
			}
		...
		<div class="nextprev">
			<ul>
				...
				<li><span class="opt fi">첨부파일</span><div class="value"><a href="../../upfiles/bbs/<?=$r['b_file']?>" class="fi_link"><?=$r['b_file']?></a></div></li>
				<? } ?>
				<li>
					<?
					...
					?>
					<a href="<?=$prev_link?>" <?=$prev_link=='#none'?"onclick='return false;'":""?> class="upper_link"></a>
					<span class="opt pv"><span class="shape"></span>이전글</span>
					<div class="value"><?=$prev_title?></div>
				</li>
				<li>
					<?
					...
					?>
					<a href="<?=$next_link?>" <?=$next_link=='#none'?"onclick='return false;'":""?> class="upper_link"></a>
					<span class="opt nx"><span class="shape"></span>다음글</span>
					<div class="value"><?=$next_title?></div>
				</li>
			</ul>
		</div>
		<!-- / 이전글 다음글 -->
	...
	<div class="cm_bottom_button">
		<ul>
			<li><span class="button_pack"><a href="/?<?=$_PVSC?enc('d',$_PVSC):"pn=board.list&_menu=".$b_menu?>" class="btn_md_white">목록으로<span class="edge"></span></a></span></li>
			...
			<li><span class="button_pack"><a href="/?pn=board.form&_mode=reply&_menu=<?=$r['b_menu']?>&_uid=<?=$r['b_uid']?>&_PVSC=<?=$_PVSC?>" class="btn_md_color">답글쓰기<span class="edge"></span></a></span></li>
			<? } ?>
		</ul>
		<?if($is_auth || $r['b_inid'] == get_userid()) {?>
		<ul>
			<li><span class="button_pack"><a href="/?pn=board.form&_mode=modify&_menu=<?=$r['b_menu']?>&_uid=<?=$r['b_uid']?>&_PVSC=<?=$_PVSC?>" class="btn_md_black">수정<span class="edge"></span></a></span></li>
			<li><span class="button_pack"><a href="#none" onclick="<?=$delete_event?>;return false;" class="btn_md_black">삭제<span class="edge"></span></a></span></li>
		</ul>
		<? } ?>
	</div>
	<!-- / 가운데정렬버튼 -->
